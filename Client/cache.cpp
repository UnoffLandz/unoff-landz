 ...
 * void cache_set_size(cache_struct*, uint32_t, void*);
 * void cache_use_item(cache_struct*, const void*);
 * void cache_remove_item(cache_struct*, cosnt uint8_t*);
...
static uint32_t cache_system_clean();
static uint32_t cache_system_compact();
static uint32_t cache_clean(cache_struct *cache);
static uint32_t cache_compact(cache_struct *cache);
...
static int cache_item_cmp_str(const void* str, const void *iptr)
{
    ...
    if (!item) return -1;
    return strcmp((const char *)str, item->name);
}
#endif
...
{
    cache_system = cache_init("", max_items, (void (*)(void *))&cache_delete);
...
void cache_dump_sizes(const cache_struct *cache)
{
...
#endif
    {
        ...
        if (item)
        {
            ...
            {
                cache_struct *temp = (cache_struct *)item->cache_item;
            ...
            log_error(str);
#else
            write_to_log(CHAT_SERVER, str, strlen(str));
#endif
        }
    }
}
...
static uint32_t cache_system_clean()
{
...
#endif
    {
        ...
        if (item && item->cache_item)
            mem_freed += cache_clean((cache_struct *)item->cache_item);
    }
...
static uint32_t cache_system_compact()
{
...
#endif
    {
        item = cache_system->cached_items[i];
        if (item && item->cache_item)
            mem_freed += cache_compact((cache_struct *) item->cache_item);
    }
    ...
    void (*free_item)(void *))
{
    ...
    cache = (cache_struct *)calloc(1, sizeof(cache_struct));
    if(!cache)
        return NULL;	//oops, not enough memory

    cache->cached_items = (cache_item_struct **)calloc(max_items, sizeof(cache_item_struct *));
...
#ifdef FASTER_MAP_LOAD
    item = (cache_item_struct **)bsearch(name, cache->cached_items, cache->num_items,
    ...
    void *item, uint32_t size)
{
    ...
    new_item = (cache_item_struct *)calloc(1, sizeof(cache_item_struct));
    if (!new_item)
        return NULL;
...
void cache_set_size(cache_struct *cache, uint32_t size, void *item)
{
...
void cache_remove_item(cache_struct *cache, const uint8_t *name)
{
    cache_item_struct *item;

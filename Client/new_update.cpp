...
static uint32_t download_file(const char* file_name, FILE* file,
    const char* server, const char* path, uint64_t* file_size,
    const uint32_t size, char* buffer, const uint32_t etag_size,
    char* etag)
{
    char str[64];
    char* pos;
    IPaddress http_ip;
    TCPsocket http_sock;
    uint32_t got_header, http_status;
    int i, len;
...
static int download_files_thread(void* _data)
{
    ...
    uint32_t i, len, result, error, count, index, running;
    ...
    while (1)
    {
        ...
        do
        {
            info = queue_pop_T<update_info_t>(data->files);
        ...
        for (i = 0; i < 5; i++)
        {
            ...
            add_to_zip(file_name, file_size, (const uint8_t*)file_buffer,
                data->dest, comment);
            data->index++;
...
static void init_threads(download_files_thread_data_t *data, const uint32_t count,
...
static void wait_for_threads(download_files_thread_data_t *data, uint32_t* error)
{
    int32_t result;
    uint32_t i;
...
static uint32_t download_files(update_info_t* infos, const uint32_t count,
    const char* server, const char* path, const uint32_t source_count,
    ...
    void* user_data)
{
    ...
    FILE *file;
    uint64_t len;
    uint32_t i, j, download, error, index;
...
static uint64_t skip_line(const char** buffer, uint64_t* size, uint64_t* skip)
{
    uint64_t len, i;
...
static uint32_t add_to_downloads(const char* buffer, const uint64_t buffer_size,
    update_info_t** infos, uint32_t* count,
    progress_fnc update_progress_function, void* user_data)
{
    char name[256];
    char md5[33];
    ...
    uint64_t line_size, line_buffer_size, skip;
    uint32_t size;
    ...
    while (line_buffer_size > 0)
    {
            ...
            (name[1] != ':'))
        {
            ...
            {
                size = *count + 1024;
                *infos = (update_info_t *)realloc(*infos, size * sizeof(update_info_t));
            }
...
static uint32_t check_server_digest_file(const char* file, FILE* tmp_file,
    const char* server, const char* path, const uint32_t size, char* buffer,
    const uint32_t digest_size, char* digest)
{
    uint64_t file_size;
    uint32_t result;
...
static uint32_t check_server_digest_files(const char* file, FILE* tmp_file,
    const char* server, const char* path, const uint32_t size, char* buffer,
    char md5[33])
{
    char *file_name = NULL;
    ...
    if ((file_name = (char *)calloc(sizeof(char), file_name_size)) == NULL)
        return 0;
...
static uint32_t build_update_list(const char* server, const char* file,
    const char* path, update_info_t** infos, uint32_t* count, char md5[33],
    const uint32_t etag_size, char* etag,
    progress_fnc update_progress_function, void* user_data)
{
    char *buffer = NULL;
    ...
    *count = 0;
    ...
    if (add_to_downloads((const char *)file_buffer, file_size, infos, count,

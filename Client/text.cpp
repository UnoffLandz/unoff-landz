...
int last_message = -1;
uint8_t current_filter = FILTER_ALL;

text_message input_text_line;

char last_pm_from[32];

uint32_t last_server_message_time;
int lines_to_show=0;
...
/* forward declaration */
void put_small_colored_text_in_box (uint8_t color, const char *text_to_add, int len, int pixels_limit, char *buffer);

void alloc_text_message_data (text_message *msg, int size)
{
    msg->data = size > 0 ? (char *)calloc (size, 1) : NULL;
    msg->size = size;
    msg->len = 0;
}
...
void resize_text_message_data (text_message *msg, int len)
{
    if (msg->size <= len)
    {
        ...
        while (nsize < len)
            nsize += nsize;
        msg->data = (char *)realloc (msg->data, nsize);
        msg->size = nsize;
        }
}
...
void write_to_log (uint8_t channel, const char* const data, uint32_t len)
{
    int j;
    uint8_t ch;
    char str[1024];
...
void send_input_text_line (const char *line, int line_len)
{
    char str[256];
    int i,j;
    int len;
    ...
    char *transformed_line = (char *)alloca(line_len);
    strncpy(transformed_line,line,line_len);
    if ( caps_filter && line_len > 4 && my_isupper (transformed_line, -1) )
        my_tolower (transformed_line);

    i=0;
    j=1;
    if (line[0] != '/' && transformed_line[0] != char_slash_str[0])	//we don't have a PM
    {
        str[0] = RAW_TEXT;
    }
    else
    ...
    len = strlen (&str[1]);
    if (my_tcp_send (my_socket, (uint8_t*)str, len+1) < len+1)
    {
...
int filter_or_ignore_text (char *text_to_add, int len, int size, uint8_t channel)
{
    int l, idx;
...
void put_text_in_buffer (uint8_t channel, const char *text_to_add, int len)
{
...
void check_chat_text_to_overtext (const char *text_to_add, int len, uint8_t channel)
{
...
int put_char_in_buffer (text_message *buf, uint8_t ch, int pos)
{
    int i, nlen;
...
int put_string_in_buffer (text_message *buf, const char *str, int pos)
{
...
void put_colored_text_in_buffer (uint8_t color, uint8_t channel, const char *text_to_add, int len)
{
    text_message *msg;
    int minlen, text_color;
    uint32_t cnr = 0, ibreak = -1, jbreak = -1;
    char time_stamp[12];
    struct tm *l_time; time_t c_time;
    ...
    // log the message
    write_to_log (channel, msg->data, msg->len);

    return;
}

void put_small_text_in_box (const char *text_to_add, int len, int pixels_limit, char *buffer)
{
...
void put_small_colored_text_in_box (uint8_t color, const char *text_to_add, int len, int pixels_limit, char *buffer)
{
    ...
    else //we have to add new lines to our text...
    {
        ...
        int new_line_pos = 0;
        char semaphore = 0;
        uint8_t current_color = to_color_char (color);
...
int find_last_lines_time (int *msg, int *offset, uint8_t filter, int width)
{
...
int find_line_nr (int nr_lines, int line, uint8_t filter, int *msg, int *offset, float zoom, int width)
{

...
int IMG_SavePNG_RW (SDL_Surface *face, SDL_RWops *src)
{
    ...
    for (i = 0; i < face->h; i++)
        row_pointers[i] = (png_bytep)(uint8_t *)face->pixels + i*face->pitch;
...
void makeScreenShot ()
{
    ...
    pixels = (uint8_t *)malloc(h * align * ((3 * w - 1) / align + 1));
...
void draw_smooth_button(char * str, float size, int x, int y, int w, int lines, float r, float g, float b, int highlight, float hr, float hg, float hb, float ha)
{
    ...
    if(str) {
        draw_string_zoomed(xstr + gx_adjust, y+radius/2.0f + gy_adjust, str, lines, size);
    }
#ifdef OPENGL_TRACE
CHECK_GL_ERRORS();
#endif //OPENGL_TRACE

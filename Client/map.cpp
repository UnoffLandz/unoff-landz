...
int load_empty_map()
{
    if (!el_load_map("./maps/nomap.elm"))
    {
        ...
        ambient_g = 0;
        ambient_b = 0;
        tile_map = (uint8_t *)calloc(tile_map_size_x*tile_map_size_y, sizeof(char));
        height_map = (uint8_t *)calloc(tile_map_size_x*tile_map_size_y*6*6, sizeof(char));
#ifndef MAP_EDITOR2
        pf_tile_map = (PF_TILE *)calloc(tile_map_size_x*tile_map_size_y*6*6, sizeof(PF_TILE));
#endif
        return 0;
...
void load_server_markings(){
    ...
    while((rf=fscanf(fp,"%d %d %d %s %[^\n]s\n",&sm.id,&sm.x,&sm.y,sm.map_name,sm.text))==5){
        server_mark *nm = (server_mark *)calloc(1,sizeof(server_mark));
        memcpy(nm,&sm,sizeof(server_mark));
        hash_add(server_marks,(void *)sm.id,(void*) nm);
    }
...
int get_3d_objects_from_server (int nr_objs, const uint8_t *data, int len)
{
    ...
    for (iobj = 0; iobj < nr_objs; iobj++)
    {
        ...
        obj_x = SDL_SwapLE16 (*((uint16_t *)(&data[offset])));
        offset += 2;
        obj_y = SDL_SwapLE16 (*((uint16_t *)(&data[offset])));
        offset += 2;
        ...
        else
        {
            ...
            offset += 2;
            id = SDL_SwapLE16 (*((uint16_t *)(&data[offset])));
            offset += 2;
...
void display_map_markers() {
    ...
    for(i=0;i<max_mark;i++){
        ...
        banner_width = ((float)get_string_width(marks[i].text)*(font_size_x*name_zoom))/2.0;
        draw_ortho_ingame_string(hx-banner_width, hy, hz, marks[i].text, 4, font_size_x, font_size_y);
        //restore text

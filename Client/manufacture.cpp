...
static uint32_t last_recipe_key_time = 0;
...
static int item_ids_match(uint16_t lid, uint16_t rid)
{
 ...
 * value for unset uids is (uint16_t)-1 so this needs fixing.  From now on
...
static void draw_recipe_controls(){
    ...
    draw_string_zoomed(wpx+3, wpy-2, "+", 1, 1);
}
...
static int draw_production_pipe(int x, int y, int recipe_num){
    ...
    for(i=ofs;i<NUM_MIX_SLOTS+ofs;i++) {
        ...
        if(the_list[i].quantity > 0){
            ...
            safe_snprintf(str, sizeof(str), "%i", the_list[i].quantity);
    ...
    for(i=35;i>=0;i--){
        if(manufacture_list[i].quantity > 0) {
            ...
            draw_string_small_shadowed(x_start, (i&1)?(y_start+17):(y_start+7),
                str, 1,1.0f,1.0f,1.0f, 0.0f, 0.0f, 0.0f);
        }
    }
...
static int keypress_recipe_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
static int keypress_manufacture_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
//CLICK HANDLERS
static int recipe_dropdown_click_handler(window_info *win, int mx, int my, uint32_t flags){
...
static int recipe_controls_click_handler(int mx, int my, uint32_t flags){
...
static int click_manufacture_handler(window_info *win, int mx, int my, uint32_t flags)
{
    int pos;
    uint8_t str[100];
...
int mix_handler(uint8_t quantity, const char* empty_error_str)
{
    uint8_t str[32];
    int items_no=0;
    int i;
    ...
    for(i=MIX_SLOT_OFFSET;i<MIX_SLOT_OFFSET+NUM_MIX_SLOTS;i++){
        if(manufacture_list[i].quantity > 0){
            ...
            *((uint16_t *)(str+items_no*3+2+1))=SDL_SwapLE16(manufacture_list[i].quantity);
            items_no++;
        }
    }
...
static int clear_handler(widget_list *widget, int mx, int my,uint32_t flags)
{
    int i;
...
static int mixone_handler(widget_list *widget, int mx, int my,uint32_t flags)
{
  return mix_handler(1, mix_empty_str);
}

/* ">>" button clicked - manufacture the maxiumum nuber of items */
int mixall_handler(widget_list *, int, int, uint32_t)
{
...
void display_manufacture_menu()
{
    if(manufacture_win < 0){
        ...
        set_window_handler(manufacture_win, ELW_HANDLER_CLICK, (int (*)())&click_manufacture_handler );
        set_window_handler(manufacture_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_manufacture_slot_handler );
        ...
        set_window_handler(recipe_win, ELW_HANDLER_CLICK, (int (*)())&recipe_dropdown_click_handler );
        set_window_handler(recipe_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_recipe_handler );
        set_window_handler(recipe_win, ELW_HANDLER_RESIZE, (int (*)())&resize_recipe_handler );

...
static uint8_t storage_items_filter[STORAGE_ITEMS_SIZE];

//	Look though the category for the selected item, pick it up if found.
//
static void select_item(int image_id, uint16_t item_id)
{
    int i;
    int found_at = -1;
...
static uint16_t wanted_item_id = -1;
...
void get_storage_categories (const char *in_data, int len)
{
    ...
    {
        storage_categories[i].id = (uint8_t)in_data[idx++];
...
void set_window_name(const char *extra_sep, const char *extra_name)
{
...
void move_to_category(int cat)
{
    ...
    *((uint8_t *)(str+1))=storage_categories[cat].id;
...
void get_storage_items (const uint8_t *in_data, int len)
{
    ...
    if (in_data[0] == 255)
    {
        ...
        idx = 2;
        active_storage_item = SDL_SwapLE16(*((uint16_t*)(&in_data[idx+6])));

        for (i = 0; i < STORAGE_ITEMS_SIZE; i++)
        {
            if ((storage_items[i].pos == SDL_SwapLE16(*((uint16_t*)(&in_data[idx+6])))) && (storage_items[i].quantity > 0))
            {
                storage_items[i].image_id = SDL_SwapLE16(*((uint16_t*)(&in_data[idx])));
                storage_items[i].quantity = SDL_SwapLE32(*((uint32_t*)(&in_data[idx+2])));
                if (item_uid_enabled)
                    storage_items[i].id = SDL_SwapLE16(*((uint16_t*)(&in_data[idx+8])));
                else
        ...
        for (i = 0; i < STORAGE_ITEMS_SIZE; i++)
        {
            if (storage_items[i].quantity == 0)
            {
                if (item_uid_enabled)
                    ...
                    storage_items[i].id = unset_item_uid;
                storage_items[i].pos = SDL_SwapLE16(*((uint16_t*)(&in_data[idx+6])));
                storage_items[i].image_id = SDL_SwapLE16(*((uint16_t*)(&in_data[idx])));
                storage_items[i].quantity = SDL_SwapLE32(*((uint32_t*)(&in_data[idx+2])));
                no_storage++;
                update_item_filter();
...
int display_storage_handler(window_info * win)
{
    ...
    for(i=pos=vscrollbar_get_pos(storage_win,STORAGE_SCROLLBAR_CATEGORIES); i<no_storage_categories && storage_categories[i].id!=-1 && i<pos+STORAGE_CATEGORIES_DISPLAY; i++,n++){
        ...
        if (the_colour == c_red3)
            ...
            offset = 0;
        draw_string_small(20, 20+n*13, &storage_categories[i].name[offset],1);
    }
    glColor3f(0.77f, 0.57f, 0.39f);
    if(storage_text[0]){
        if (strcmp(storage_text, last_storage_text) != 0) {
            safe_strncpy(last_storage_text, storage_text, sizeof(last_storage_text));
            put_small_text_in_box (storage_text, strlen(storage_text), win->len_x - 18*2, wrapped_storage_text);
        }
        draw_string_small(18, 220, wrapped_storage_text, 2);
    }
    ...
    if(cur_item_over!=-1 && mouse_in_window(win->window_id, mouse_x, mouse_y) == 1){
        char str[20];
        uint16_t item_id = storage_items[cur_item_over].id;
...
int click_storage_handler(window_info * win, int mx, int my, uint32_t flags)
{
                ...
                if(view_only_storage && item_dragged!=-1 && left_click){
                    ...
                    *((uint32_t*)(str+2))=SDL_SwapLE32(item_quantity);
                    ...
                    if(cur_item_over!=-1) {
                        ...
                        *((uint16_t*)(str+1))=SDL_SwapLE16(storage_items[cur_item_over].pos);
...
static int keypress_storage_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
void print_items(void)
{
    ...
    for (i = 0; i < no_storage && i < STORAGE_ITEMS_SIZE; i++)
    {
        if (storage_items[i].quantity)
        {
            ...
            str[0]=LOOK_AT_STORAGE_ITEM;
            *((uint16_t*)(str+1))=SDL_SwapLE16(storage_items[i].pos);
            my_tcp_send(my_socket, str, 3);
        }
    }
}
...
void display_storage_menu()
{
    ...
    if(storage_win<=0){
        ...
        set_window_handler(storage_win, ELW_HANDLER_CLICK, (int (*)())&click_storage_handler);
        set_window_handler(storage_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_storage_handler);

 ...
 *
 * int32_t get_string_after_string(const uint8_t*, const uint8_t*, int32_t, Uint*, int);
 */
...
{
    int32_t n_end = get_string_occurance (needle, haystack, max_len, 0);
    uint32_t istart;
...
int get_file_digest(const char * filename, uint8_t digest[16])
{
    MD5 md5;
...
int32_t get_string_after_string(const uint8_t * source_pointer, const uint8_t * dest_pointer, int32_t max_len, uint8_t *value, int32_t value_len)
{
    int i;
...
void get_string_digest(const uint8_t * string, uint8_t digest[16])
{
    MD5 md5;
    MD5Open(&md5);
...
void get_item_string_value(char *buf, size_t maxlen, const xmlNode *item, const char *name)
{
    const xmlNode *node;
...
xmlChar* toUTF8 (const char* str, int len)
{
    int out_size = 2*len;
    int out_len;
    xmlChar* out = (xmlChar *)calloc (out_size, sizeof (xmlChar));
    ...
    while (1)
    {
        ...
        out_size *= 2;
        out = (xmlChar *)realloc (out, out_size * sizeof (xmlChar));
    }

    if (out_len >= out_size)
        // drats, no space to store a terminator
        out = (xmlChar *)realloc (out, (out_size + 1) * sizeof (xmlChar));
    out[out_len] = '\0';
...
char *truncated_string(char *dest, const char *source, size_t dest_max_len, const char *append_str, float max_len_x, float font_ratio)
{
    ...
    size_t dest_len = 0;
    float append_len_x = get_string_width(append_str) * font_ratio;
    char *dest_p = dest;

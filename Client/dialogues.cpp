...
static uint32_t copy_end_highlight_time = 0;
static uint32_t repeat_end_highlight_time = 0;
...
void build_response_entries (const uint8_t *data, int total_length)
{
    ...
    for(i=0;i<MAX_RESPONSES;i++)
    {
        ...
        if (last_index + 3 > total_length)
            break;
        orig_len=len=SDL_SwapLE16(*((uint16_t *)(data+last_index)));
        ...
        dialogue_responces[i].response_id=SDL_SwapLE16(*((uint16_t *)(data+last_index+2+len)));
        dialogue_responces[i].to_actor=SDL_SwapLE16(*((uint16_t *)(data+last_index+2+2+len)));
...
static int	display_dialogue_handler(window_info *win)
{
    ...
    if (highlight_close)
        ...
        glColor3f(1.0f,1.0f,1.0f);
    draw_string_small(win->len_x-(str_edge+close_str_width),win->len_y-(SMALL_FONT_Y_LEN+1),close_str,1);
    ...
    if (copy_end_highlight_time > SDL_GetTicks())
        ...
        glColor3f(1.0f,1.0f,1.0f);
    draw_string_small(str_edge,win->len_y-(SMALL_FONT_Y_LEN+1),dialogue_copy_str,1);
    ...
    if (!saved_response_init)
        ...
        glColor3f(1.0f,1.0f,1.0f);
    draw_string_small(4*str_edge+copy_str_width,win->len_y-(SMALL_FONT_Y_LEN+1),dialogue_repeat_str,1);
...
static void send_response(window_info *win, const response *the_response)
{
    ...
    *((uint16_t *)(str+1))=SDL_SwapLE16((short)the_response->to_actor);
    *((uint16_t *)(str+3))=SDL_SwapLE16((short)the_response->response_id);
...
static int click_dialogue_handler(window_info *win, int mx, int my, uint32_t flags)
{
    int i;
...
static int keypress_dialogue_handler (window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
    uint8_t ch;
    ...
    {
        if ((strlen(dialogue_repeat_str)>1) && (ch == (uint8_t)dialogue_repeat_str[1]-87))
        {
            send_repeat(win);
            return 1;
        }
        if ((strlen(dialogue_copy_str)>1) && (ch == (uint8_t)dialogue_copy_str[1]-87))
        {
            do_copy();
            return 1;
        }
...
void display_dialogue()
{
    ...
    if(dialogue_win < 0){
        ...
        set_window_handler(dialogue_win, ELW_HANDLER_MOUSEOVER,(int (*)()) &mouseover_dialogue_handler );
        set_window_handler(dialogue_win, ELW_HANDLER_KEYPRESS, (int (*)())&keypress_dialogue_handler );
        set_window_handler(dialogue_win, ELW_HANDLER_CLICK, (int (*)())&click_dialogue_handler );
        ...
        copy_str_width = get_string_width(dialogue_copy_str) * SMALL_FONT_X_LEN / 12.0;
        close_str_width = get_string_width(close_str) * SMALL_FONT_X_LEN / 12.0;
        repeat_str_width = get_string_width(dialogue_repeat_str) * SMALL_FONT_X_LEN / 12.0;
    } else {

...
int get_nstring_width(const char *str, int len);
...
int find_font_char (uint8_t cur_char)
{
...
int	draw_char_scaled(uint8_t cur_char, int cur_x, int cur_y, float displayed_font_x_size, float displayed_font_y_size)
{
...
void draw_messages (int x, int y, text_message *msgs, int msgs_size, uint8_t filter, int msg_start, int offset_start, int cursor, int width, int height, float text_zoom, select_info* select)
{
...
void draw_ortho_ingame_string(float x, float y,float z, const char * our_string,
...
void draw_ingame_string(float x, float y,const char * our_string,
                        int max_lines, float font_x_scale, float font_y_scale)
{
    float u_start,u_end,v_start,v_end;
    int col,row;
...
int get_nstring_width(const char *str, int len)
{
    ...
    for(i=0; i<len; i++) {
        wdt+= get_char_width(((const unsigned char *)str)[i]);
    }
...
int load_font_textures ()
{
    ...
    glob_pattern = (char *)malloc(strlen(datadir)+sizeof(texture_dir)+10+1); //+10 = font*.bmp*
#ifdef	NEW_TEXTURES

...
#ifdef ELC
#define DRAW_ORTHO_INGAME_NORMAL(x, y, z, our_string, max_lines)	draw_ortho_ingame_string(x, y, z, (const char*)our_string, max_lines, INGAME_FONT_X_LEN*10.0, INGAME_FONT_Y_LEN*10.0)
#define DRAW_INGAME_NORMAL(x, y, our_string, max_lines)	draw_ingame_string(x, y, (const char*)our_string, max_lines, INGAME_FONT_X_LEN, INGAME_FONT_Y_LEN)
#define DRAW_INGAME_SMALL(x, y, our_string, max_lines)	draw_ingame_string(x, y, (const char*)our_string, max_lines, SMALL_INGAME_FONT_X_LEN, SMALL_INGAME_FONT_Y_LEN)
#define DRAW_INGAME_ALT(x, y, our_string, max_lines)	draw_ingame_string(x, y, (const char*)our_string, max_lines, ALT_INGAME_FONT_X_LEN, ALT_INGAME_FONT_Y_LEN)
#endif
...
int add_actor (int actor_type, char * skin_name, float x_pos, float y_pos, float z_pos, float z_rot, float scale, char remappable, short skin_color, short hair_color, short eyes_color, short shirt_color, short pants_color, short boots_color, int actor_id)
{
    ...
    our_actor = (actor *)calloc(1, sizeof(actor));
    ...
    if (!((first_person)&&(actor_id->actor_id==yourself)))
    {
        if(actor_id->actor_name[0] && (display_names || display_health_line || display_ether_line)){
            ...
            if(display_names){
                ...
                banner_width = ((float)get_string_width(actor_id->actor_name)*(font_size_x*name_zoom))/2.0;
                draw_ortho_ingame_string(hx-banner_width, hy+bar_y_len/2.0f, hz, temp, 1, font_size_x, font_size_y);
            }
            if (view_buffs)
            {
                draw_buffs(actor_id->actor_id, (float)hx, (float)hy, (float)hz);
            }
...
{
    int32_t i, has_alpha, has_ghosts;
    uint32_t use_lightning = 0, use_textures = 0;
...
void add_actor_from_server (const char *in_data, int len)
{
    ...
    if(len > 17+(int)strlen(in_data+17)+2){
        ...
        if(len > 17+(int)strlen(in_data+17)+3)
            attachment_type = (uint8_t)in_data[17+strlen(in_data+17)+3];
    }
    ...
    default:
        ...
        if(frame>=frame_poses_start&&frame<=frame_poses_end) {
            ...
            hash_entry *he;
            he=hash_get(emotes,(void*)(frame));
            if(!he)
                LOG_ERROR("unknown pose %d", frame);
            else
                pose = (emote_data *)he->item;
            break;
        }

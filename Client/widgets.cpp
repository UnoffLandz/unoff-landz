...
int widget_destroy (int window_id, uint32_t widget_id)
{
    ...
    if (n->id == widget_id)
    {
        ...
        {
            if (n->spec != NULL)
                ((int (*)(widget_list *,void * ))n->OnDestroy) (n, n->spec);
            else
                n->OnDestroy (n);
        }
        if (n->type != NULL)
...
int widget_set_OnDraw(int window_id, uint32_t widget_id, int (*handler)(widget_list *))
{
...
int widget_set_OnClick(int window_id, uint32_t widget_id, int (*handler)(widget_list *widget, int mx, int my, uint32_t flags))
{
    widget_list *w = widget_find(window_id, widget_id);
    if(w){
        w->OnClick = handler;
...
int widget_set_OnMouseover(int window_id, uint32_t widget_id, int (*handler)(widget_list *,int mx, int my))
{
    widget_list *w = widget_find(window_id, widget_id);
    if(w){
...
int widget_set_OnKey ( int window_id, uint32_t widget_id, int (*handler)(widget_list *widget, int mx, int my, uint32_t key, uint32_t unikey) )
{
    widget_list *w = widget_find (window_id, widget_id);
    if (w)
    {
...
int widget_set_OnDestroy (int window_id, uint32_t widget_id, int (*handler)(struct widget_list *))
{
    widget_list *w = widget_find(window_id, widget_id);
    if(w)
    {
...
int widget_resize(int window_id, uint32_t widget_id, uint16_t x, uint16_t y)
{
    ...
    if (w)
    {
        ...
        {
            if(w->spec != NULL)
                res |= ((int (*)(widget_list *,int,int,void *))w->OnResize) (w, x, y, w->spec);
            else
                res |= w->OnResize (w, x, y);
        }
...
uint32_t widget_add (int window_id, uint32_t wid, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, uint16_t lx, uint16_t ly,
    uint32_t Flags, float size, float r, float g, float b, const struct WIDGET_TYPE *type, void *T, void *S)
{
    ...
    {
        if(W->spec != NULL)
            ((int (*)(widget_list *,void *))W->OnInit) (W, W->spec);
        else
            W->OnInit (W);
    }
...
int widget_handle_mouseover (widget_list *widget, int mx, int my)
{
    ...
    if (widget->OnMouseover != NULL && res != -1) {
        if(widget->spec != NULL) {
            res |= ((int (*)(widget_list *,int, int,void *))widget->OnMouseover) (widget, mx, my, widget->spec);
        } else {
...
int widget_handle_click (widget_list *widget, int mx, int my, uint32_t flags)
{
    ...
    {
        if(widget->spec != NULL) {
            res |= ((int (*)(widget_list *,int, int,uint32_t,void *))widget->OnClick) (widget, mx, my, flags,  widget->spec);
        } else {
...
int widget_handle_drag (widget_list *widget, int mx, int my, uint32_t flags, int dx, int dy)
{
    ...
    {
        if(widget->spec != NULL) {
            res |= ((int (*)(widget_list *,int, int,uint32_t,int,int,void *))widget->OnDrag) (widget, mx, my, flags, dx, dy, widget->spec);
        } else {
...
int widget_handle_keypress (widget_list *widget, int mx, int my, uint32_t key, uint32_t unikey)
{
    ...
    {
        if(widget->spec != NULL) {
            res |= ((int (*)(widget_list *, int, int, uint32_t, uint32_t,void *))widget->OnKey) (widget, mx, my, key, unikey, widget->spec);
        } else {
...
{
    uint16_t len_x = (uint16_t)(strlen (text) * 11 * 1.0);
    uint16_t len_y = (uint16_t)(18 * 1.0);
...
int label_draw(widget_list *W)
{
    ...
    draw_string_zoomed(W->pos_x,W->pos_y,l->text,1,W->size);
    return 1;
}
...
{
    image *T = (image *)calloc (1, sizeof (image));
    T->u1 = u1;
    T->u2 = u2;
#ifdef	NEW_TEXTURES
    T->v1 = -v1;
...
int checkbox_click (widget_list *W, int mx, int my, uint32_t flags)
{
    checkbox *c = (checkbox *)W->widget_info;
...
{
    checkbox *T = (checkbox *)calloc (1, sizeof (checkbox));
    T->checked = checked;
...
int checkbox_add(int window_id, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, uint16_t lx, uint16_t ly, int *checked)
{
    if(checked == NULL)
    {
        checked = (int *)calloc(1,sizeof(*checked));
    }
...
{
    uint16_t len_x = lx > 0 ? lx : (uint16_t)(strlen(text) * 11 * size) + 2*BUTTONRADIUS*size;
    uint16_t len_y = ly > 0 ? ly : (uint16_t)(18 * size) + 12*size;

    button *T = (button *)calloc (1, sizeof(button));
    safe_snprintf (T->text, sizeof(T->text), "%s", text);
...
/*	float extra_space = (W->len_x - get_string_width((uint8_t*)l->text)*W->size)/2.0f;
    if(extra_space < 0) {
        extra_space = 0;
    }*/
...
int square_button_draw(widget_list *W)
{
    ...
    glEnable(GL_TEXTURE_2D);
    draw_string_zoomed(W->pos_x + 2 + extra_space + gx_adjust, W->pos_y + 2 + gy_adjust, l->text, 1, W->size);
#ifdef OPENGL_TRACE
CHECK_GL_ERRORS();
#endif //OPENGL_TRACE
...
{
    progressbar *T = (progressbar*) calloc (1, sizeof(progressbar));
    T->progress = progress;
    if (colors) {
...
int vscrollbar_click (widget_list *W, int mx, int my, uint32_t flags)
{
...
int vscrollbar_scroll_up(int window_id, uint32_t widget_id)
{
    widget_list *w = widget_find(window_id, widget_id);
    if(w){
        vscrollbar *scrollbar = (vscrollbar *)w->widget_info;
...
int vscrollbar_drag(widget_list *W, int x, int y, uint32_t flags, int dx, int dy)
{
...
{
    vscrollbar *T = (vscrollbar *)calloc (1, sizeof(vscrollbar));
    T->pos_inc = pos_inc;
    T->pos = pos;
...
int tab_collection_close_tab (int window_id, uint32_t widget_id, int tab)
{
    widget_list *w = widget_find (window_id, widget_id);
    if (w)
        return _tab_collection_close_tab_real ((tab_collection *)w->widget_info, tab);
    return -1;
}
...
int tab_collection_draw (widget_list *w)
{
    ...
    for (itab = col->tab_offset; itab < col->nr_tabs; itab++)
    {
        ...
        if (col->tabs[itab].closable)
            draw_string_zoomed (xstart+h+gx_adjust, ytagbot-SMALL_FONT_Y_LEN-2+gy_adjust, col->tabs[itab].label, 1, w->size);
        else
            draw_string_zoomed (xstart+4+gx_adjust, ytagbot-SMALL_FONT_Y_LEN-2+gy_adjust, col->tabs[itab].label, 1, w->size);
        glDisable(GL_TEXTURE_2D);
...
int tab_collection_click (widget_list *W, int x, int y, uint32_t flags)
{
    tab_collection *col = (tab_collection *) W->widget_info;
...
int tab_collection_resize (widget_list *W, uint32_t width, uint32_t height)
{
    uint16_t win_height;
    tab_collection *col;
    int itab;
...
int tab_collection_keypress (widget_list *W, int mx, int my, uint32_t key, uint32_t unikey)
{
...
int tab_collection_add_extended (int window_id, uint32_t wid, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, uint16_t lx, uint16_t ly, uint32_t Flags, float size, float r, float g, float b, int max_tabs, uint16_t tag_height)
{
    int itab;
    tab_collection *T = (tab_collection *)calloc (1, sizeof (tab_collection));
    T->max_tabs =  max_tabs <= 0 ? 2 : max_tabs;
    T->tabs = (tab *)calloc (T->max_tabs, sizeof (tab));
...
int tab_collection_add (int window_id, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, uint16_t lx, uint16_t ly, uint16_t tag_height)
{
...
int tab_add (int window_id, uint32_t col_id, const char *label, uint16_t tag_width, int closable, uint32_t flags)
{
    ...
    if (nr >= col->max_tabs)
    {
        ...
        col->tabs = (tab *)realloc ( col->tabs, new_max * sizeof (tab) );
...
void _text_field_insert_char (widget_list *w, char ch)
{
    ...
    if (msg->len >= msg->size-2)
    {
        ...
        {
            msg->size *= 2;
            msg->data = (char *)realloc (msg->data, msg->size * sizeof (char) );
        }
    }
...
int text_field_resize (widget_list *w, uint32_t width, uint32_t height)
{
    text_field *tf = (text_field *)w->widget_info;
    ...
    if (tf != NULL)
    {
        ...
        if (tf->buffer != NULL)
        {
            ...
            if (tf->nr_visible_lines != nr_vis)
                tf->select.lines = (text_field_line *)realloc (tf->select.lines, tf->nr_visible_lines * sizeof (text_field_line));
...
static void text_widget_insert(const char *thestring)
{
    ...
    if (w != NULL)
    {
        uint32_t saved_flag = w->Flags & TEXT_FIELD_NO_KEYPRESS;
...
int text_field_click(widget_list *w, int mx, int my, uint32_t flags)
{
    uint32_t buttons;
    text_field *tf;
    int em_before;
...
int text_field_drag(widget_list *w, int mx, int my, uint32_t flags, int dx, int dy)
{
...
int text_field_add_extended (int window_id, uint32_t wid, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, uint16_t lx, uint16_t ly, uint32_t Flags, float size, float r, float g, float b, text_message *buf, int buf_size, uint8_t chan_filt, int x_space, int y_space)
{
    ...
    text_field *T = (text_field *)calloc (1, sizeof (text_field));
    T->x_space = x_space,
    T->y_space = y_space,
    T->msg = 0;
    T->offset = 0;
...
int pword_field_draw (widget_list *w)
{
    ...
    if (pword->status == P_NONE) {
        ...
        text = (char *)calloc(1, pword->max_chars);
...
{
    password_entry *T = (password_entry *)calloc (1, sizeof (password_entry));
    T->status = status;
...
{
    if(widget != NULL) {
        multiselect *collection = (multiselect *)widget->widget_info;
...
int multiselect_get_selected(int window_id, uint32_t widget_id)
{
    ...
    multiselect *M =(multiselect *) widget->widget_info;
    if(M == NULL) {
        return -1;
    } else {
...
int multiselect_click(widget_list *widget, int mx, int my, uint32_t flags)
{
    multiselect *M = (multiselect *)widget->widget_info;
    int i;
    uint16_t button_y;
    ...
    if(M->scrollbar != -1)
    {
        vscrollbar *scrollbar = (vscrollbar *)widget_find(M->win_id, M->scrollbar)->widget_info;
...
int multiselect_button_add_extended(int window_id, uint32_t multiselect_id, uint16_t x, uint16_t y, int width, const char *text, float size, const char selected)
{
    ...
    if(M->max_buttons == M->nr_buttons) {
        ...
        M->buttons = (multiselect_button *)realloc(M->buttons, sizeof(*M->buttons) * M->max_buttons * 2);
        M->max_buttons *= 2;
    }
...
int multiselect_add_extended(int window_id, uint32_t wid, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, int width, uint16_t max_height, float size, float r, float g, float b, float hr, float hg, float hb, int max_buttons)
  {
    multiselect *T =(multiselect *) calloc (1, sizeof (multiselect));
    //Save info
    T->max_buttons = max_buttons > 0 ? max_buttons : 2;
    T->selected_button = 0;
    T->next_value = 0;
    T->nr_buttons = 0;
    T->buttons = (multiselect_button *)malloc(sizeof(*T->buttons) * T->max_buttons);
...
// Spinbutton
int spinbutton_keypress(widget_list *widget, int mx, int my, uint32_t key, uint32_t unikey)
{
    if(NULL==widget)
        return 0;
    ...
    if(button != NULL &&  !(key&ELW_ALT) && !(key&ELW_CTRL)) {
...
//		handler changes.
int spinbutton_click(widget_list *widget, int mx, int my, uint32_t flags)
{
...
int spinbutton_draw(widget_list *widget)
{
    ...
    if(widget == NULL || (button = (spinbutton *)widget->widget_info) == NULL) {
        return 0;
    }
    switch(button->type) {
        case SPIN_INT:
    ...
    draw_string_zoomed(widget->pos_x + 2 + gx_adjust, widget->pos_y + 2 + gy_adjust, str, 1, widget->size);
    glDisable(GL_TEXTURE_2D);
    /* Border */
    glBegin(GL_LINE_LOOP);
...
const struct WIDGET_TYPE spinbutton_type = {
    NULL,
    spinbutton_draw,
    spinbutton_click,
    (int (*)(widget_list *,int,int,uint32_t, int,int))spinbutton_click,
    NULL,
    NULL,
    spinbutton_keypress,
    free_multiselect
    };
int spinbutton_add_extended(int window_id, uint32_t wid, int (*OnInit)(widget_list *), uint16_t x, uint16_t y, uint16_t lx, uint16_t ly, uint8_t data_type, void *data, float min, float max, float interval, float size, float r, float g, float b)
{
    float wr = r >= 0 ? r : 0.77, wg = g >= 0 ? g : 0.59, wb = b >= 0 ? b : 0.39;

    spinbutton *T = (spinbutton *)calloc (1, sizeof (spinbutton));
    // Filling the widget info
    T->data = data;
    T->max = max;
    T->min = min;
...
int ParseWindow (xmlNode *node)
{
    ...
    uint8_t name[256] = "Default";
    int pos_x = 0, pos_y = 0, size_x = 320, size_y = 240;
    uint32_t flags = ELW_WIN_DEFAULT;
    int winid;
...
int ParseWidget (xmlNode *node, int winid)
{
    ...
    uint32_t flags = 0, id = widget_id++, tid = 0, max_tabs = 0, tag_height = 0;

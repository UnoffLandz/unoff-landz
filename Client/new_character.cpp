...
int display_advice_handler (window_info *win)
{
    ...
    draw_string(sep, sep, newchar_warning, 1);
...
#else
    {
        ...
        else
        {
            ...
            glColor3f(1.0f,1.0f,1.0f);
            draw_string_small(x, y, help_str, 1);
            draw_string_small((win->len_x - (len - i - 1)*SMALL_FONT_X_LEN)/2, y + 2 + SMALL_FONT_Y_LEN, (help_str + i + 1), 1);
            help_str[i] = ' ';
        }
    }
...
#ifdef NEW_NEW_CHAR_WINDOW
    draw_string_small(68, win->len_y-hud_y+15, zoom_in_out, 1);
    draw_string_small(68, win->len_y-hud_y+32, rotate_camera, 1);
#else
...
int click_newchar_handler (window_info *win, int mx, int my, uint32_t flags)
{
...
int keypress_newchar_handler (window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
void create_newchar_root_window (void)
{
    if (newchar_root_win < 0)
    {
        ...
        set_window_handler (newchar_root_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_newchar_handler);
        set_window_handler (newchar_root_win, ELW_HANDLER_CLICK, (int (*)())&click_newchar_handler);
...
int display_namepass_handler (window_info * win)
{
    ...
    //if(display_time>cur_time){
        draw_string_small(30, 168, create_char_error_str, 8);
    //}

    return 1;
}

int keypress_namepass_handler (window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
int click_namepass_handler(window_info * win, int mx, int my, uint32_t flags)
{
...
int display_color_race_handler (window_info *win)
{
    ...
    draw_string_small(x-(get_string_width(eyes_str)*8.0f/12.0f)/2.0f, 94, eyes_str, 1);
    draw_string_small(x-(get_string_width(shirt_str)*8.0f/12.0f)/2.0f, 117, shirt_str, 1);
    draw_string_small(x-(get_string_width(pants_str)*8.0f/12.0f)/2.0f, 140, pants_str, 1);
    draw_string_small(x-(get_string_width(boots_str)*8.0f/12.0f)/2.0f, 163, boots_str, 1);
...
int click_color_race_handler (window_info *win, int mx, int my, uint32_t flags)
{
    if ( (flags & ELW_MOUSE_BUTTON) == 0) return 0;
...
int click_done_handler(widget_list *w, int mx, int my, uint32_t flags)
{
...
int click_back_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    if(w->window_id == color_race_win)
    {
        destroy_all_actors();
...
int click_namepass_field(widget_list *w, int mx, int my, uint32_t flags)
{
    active = *(int*)w->spec;
    return 1;
}
...
{
    draw_string_small(w->pos_x, w->pos_y, create_char_error_str, 8);
    return 1;
}
...
int click_newchar_book_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    if ( (flags & ELW_MOUSE_BUTTON) == 0) return 0;
...
int click_newchar_gender_handler(widget_list *w, int mx, int my, uint32_t flags)
{
...
int click_newchar_race_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    int i = multiselect_get_selected(w->window_id, w->id);
    switch(i)
    {
...
int head_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.head = dec(our_actor.def->head, our_actor.head, 1);
...
int head_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.head = inc(our_actor.def->head, our_actor.head, 1);
...
int skin_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.skin = dec(our_actor.def->skin, our_actor.skin, 1);
...
int skin_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.skin = inc(our_actor.def->skin, our_actor.skin, 1);
...
int hair_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.hair = dec(our_actor.def->hair, our_actor.hair, 1);
...
int hair_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.hair = inc(our_actor.def->hair, our_actor.hair, 1);
...
int eyes_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.eyes = dec(our_actor.def->eyes, our_actor.eyes, 1);
#ifdef NEW_EYES
    update_eyes();
...
int eyes_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.eyes = inc(our_actor.def->eyes, our_actor.eyes, 1);
#ifdef NEW_EYES
    update_eyes();
...
int shirt_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.shirt = dec(our_actor.def->shirts, our_actor.shirt, 1);
...
int shirt_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.shirt = inc(our_actor.def->shirts, our_actor.shirt, 1);
...
int pants_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.pants=dec(our_actor.def->pants, our_actor.pants, 1);
...
int pants_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.pants=inc(our_actor.def->pants, our_actor.pants, 1);
...
int boots_dec_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.boots = dec(our_actor.def->boots, our_actor.boots, 1);
...
int boots_inc_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    our_actor.boots = inc(our_actor.def->boots, our_actor.boots, 1);
...
int box_draw(widget_list *w)
{
    ...
    draw_box((char *)w->widget_info, w->pos_x, w->pos_y, w->len_x, w->len_y, 0);
#ifdef OPENGL_TRACE
CHECK_GL_ERRORS();
#endif //OPENGL_TRACE
    return 1;
...
int init_color_race_handler(window_info * win)
{
    ...
    int (*body_handlers_dec[7])(widget_list *w, int mx, int my, uint32_t flags) =
    {&head_dec_handler, &skin_dec_handler, &hair_dec_handler, &eyes_dec_handler, &shirt_dec_handler, &pants_dec_handler, &boots_dec_handler};
    int (*body_handlers_inc[7])(widget_list *w, int mx, int my, uint32_t flags) =
    {&head_inc_handler, &skin_inc_handler, &hair_inc_handler, &eyes_inc_handler, &shirt_inc_handler, &pants_inc_handler, &boots_inc_handler};
    ...
    static char data[4] = {"P2P"};
    widget_id = widget_add(win->window_id, free_widget_id++, NULL, 136, y + 8, 120, 66 + 5*sep, 0, normal, 1.0f, 0.0f, 0.0f, &box_type, data, NULL);
...
void create_newchar_hud_window(void)
{
    ...
    set_window_handler(color_race_win, ELW_HANDLER_MOUSEOVER,(int (*)()) &mouseover_color_race_handler);

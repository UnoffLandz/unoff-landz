...
#ifdef	NEW_TEXTURES
uint32_t delay_texture_item_change(actor* a, const int which_part, const int which_id)
{
    if (a == 0)
    {
        return 0;
...
void add_enhanced_actor_from_server (const char *in_data, int len)
{
    ...
    uint32_t j;
#endif
    uint32_t uniq_id; // - Post ported.... We'll come up with something later...
    uint32_t guild_id;
    ...
    uniq_id = SDL_SwapLE32(*((uint32_t*)(in_data+28)));
    ...
    if(len > 28+(int)strlen(in_data+28)+2){
        ...
        if(len > 28+(int)strlen(in_data+28)+3)
            attachment_type = (uint8_t)in_data[28+strlen(in_data+28)+3];
    }
#endif
    ...
    this_actor=(enhanced_actor*)calloc(1,sizeof(enhanced_actor));
    ...
    else
    {
        ...
        const char *name = (char*) in_data+28;
        char *ptr;
        char buffer[32];
...
{
    enhanced_actor * this_actor=(enhanced_actor *)calloc(1,sizeof(enhanced_actor));
    actor * a;

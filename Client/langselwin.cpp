...
static const char *langsel_list_error = NULL;
...
static int langsel_load_list(void)
{
    xmlDocPtr doc;
    xmlNodePtr cur;
    const char *error_prefix = "Reading langsel.xml: ";
    ...
    for (cur = cur->xmlChildrenNode; cur; cur = cur->next)
    {
        ...
        {
            const char *propstr[4] = {"TEXT", "HIGHLIGHT", "CHOSEN", "BUTTONS" };
            char *noteslines = (char*)xmlGetProp(cur, (const xmlChar *)"NOTELINES");
            int i = atoi(noteslines);
            if (i >= 0)
                langsel_num_note_lines = i;
            for (i=0; i<4; i++)
            {
                char *text = (char*)xmlGetProp(cur, (const xmlChar *)propstr[i]);
                float r = 0, g = 0, b = 0;
...
static int langsel_save_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
...
static int langsel_quit_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    /* don't use scroll wheel - leave for scroll bar */
    if ((flags & ELW_MOUSE_BUTTON) == 0)
        return 0;
...
static int langsel_scroll_click_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
...
static int langsel_scroll_drag_handler(widget_list *widget, int mx, int my, uint32_t flags, int dx, int dy)
{
    return langsel_scroll_click_handler(widget, mx, my, flags);
}


static int click_langsel_handler(window_info *win, int mx, int my, uint32_t flags)
{
...
static int langsel_keypress_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
static int langsel_display_error_handler(window_info *win)
{
    ...
    static const char *message = "The language selection file langsel.xml could\n"
    ...
    draw_string_small(10, 10, message, 6);
    draw_string_small(10, 10 + SMALL_FONT_Y_LEN * 6,
        ((langsel_list_error) ? langsel_list_error : "Unknown error"), 1);
...
static int display_langsel_handler(window_info *win)
{
    ...
    if (first_time)
    {
        ...
        for (local_head = langsel_list; local_head; local_head = local_head->next)
        {
            ...
            float str_width = get_string_width(new_lang_node->text);
    ...
    if (second_time || (langsel_chosen_node && (last_langsel_chosen_node != langsel_chosen_node)))
    {
        ...
        if (langwin_save_note)
        {
            ...
            put_small_colored_text_in_box(c_grey1, langwin_save_note,
    ...
    if (langwin_save_note)
        draw_string_small(winsep, winheight - winsep - note_height, langsel_save_note_boxed, langsel_num_note_lines);
...
int display_langsel_win(void)
{
    ...
    set_window_handler(langsel_win, ELW_HANDLER_CLICK, (int (*)())&click_langsel_handler );

...
int port= 2000;
uint8_t server_address[60];
TCPsocket my_socket= 0;
SDLNet_SocketSet set= 0;
#define MAX_TCP_BUFFER  8192
uint8_t tcp_in_data[MAX_TCP_BUFFER];
uint8_t tcp_out_data[MAX_TCP_BUFFER];
int in_data_used=0;
int tcp_out_loc= 0;
...
uint8_t	tcp_cache[256];
uint32_t	tcp_cache_len= 0;
uint32_t	tcp_cache_time= 0;
...
static uint32_t testing_server_connection_time = 0;
...
{
    uint32_t curr_game_time = get_game_time_sec();
...
const char *get_date(void (*callback)(const char *))
{
    ...
    if (!requested_date)
    {
        uint8_t protocol_name = GET_DATE;
...
{
    uint8_t new_str[1024];	//should be enough
    uint8_t command = ((const uint8_t *)str)[0];
    ...
    // movement.
    if ((command == TURN_LEFT || command == TURN_RIGHT) && on_the_move (get_our_actor ()))
        return 0;
    if (command == DROP_ITEM  && on_the_move (get_our_actor ()))
    {
    ...
    if(len < sizeof (tcp_cache))	// only if it fits
    {
        if(command==MOVE_TO ||
                ...
                command==SIT_DOWN ||
                command==HARVEST ||
                command==MANUFACTURE_THIS ||
                command==CAST_SPELL ||
                command==RESPOND_TO_NPC ||
                command==ATTACK_SOMEONE ||
                command==SEND_PM ||
                command==RAW_TEXT ||
                command==TURN_LEFT ||
                command==TURN_RIGHT)
        {
            uint32_t	time_limit= 600;

            if( command==SEND_PM || command==RAW_TEXT)time_limit=1500;
            if( command==SIT_DOWN){
            ...
            if(tcp_cache[0] == MOVE_TO && (command==TURN_LEFT || command==TURN_RIGHT)) return 0;
    ...
    if (len + 2 < MAX_TCP_BUFFER)
    {
        ...
        tcp_out_data[tcp_out_loc] = command;	//copy the protocol byte
        *((short *)(tcp_out_data+tcp_out_loc+1)) = SDL_SwapLE16((uint16_t)len);//the data length
    ...
    new_str[0] = command;	//copy the protocol byte
    *((short *)(new_str+1)) = SDL_SwapLE16((uint16_t)len);//the data length
    // copy the rest of the data
...
void process_message_from_server (const uint8_t *in_data, int data_length)
{
    ...
    case RAW_TEXT:
    {
        ...
        safe_strncpy2(text_buf, (char*)&in_data[4], sizeof(text_buf), data_length - 4);
        len = strlen(text_buf);
    ...
    case LOG_IN_OK:
    {
        ...
        my_tcp_send(my_socket, (uint8_t*)str, strlen(str+1)+1);
        break;
    }
    ...
    case HERE_YOUR_STATS:
    {
        ...
        get_the_stats((int16_t *)(in_data+3), data_length-3);
    ...
    case SEND_PARTIAL_STAT:
    {
        ...
        while (data_length >= 8)
        {
            get_partial_stat (in_data[3], SDL_SwapLE32(*((int32_t *)(in_data+4))));
            in_data+= 5;
            data_length-= 5;
        }
        update_research_rate();
    ...
    case GET_KNOWLEDGE_LIST:
    {
        ...
        size = SDL_SwapLE16(*(uint16_t *)(in_data+1))-1;
    ...
    case GET_NEW_KNOWLEDGE:
    {
        ...
        get_new_knowledge(SDL_SwapLE16(*(uint16_t *)(in_data+3)));
    }
        break;
    ...
    case INVENTORY_ITEM_TEXT:
    {
        ...
        if (my_strncompare(inventory_item_string+1, "You failed to create a[n] ", 26))
        {
            ...
            if (located)
                item_name_len = (size_t)((located - item_string)/sizeof(char));
    ...
    case SPELL_ITEM_TEXT:
    {
        ...
        put_small_text_in_box((const char *)&in_data[3], data_length-3, 6*51+100, spell_text);
    ...
    case GET_KNOWLEDGE_TEXT:
    {
        ...
        put_small_text_in_box((const char *)&in_data[3],data_length-3,6*51+150,knowledge_string);
    }
        break;
    ...
    case GET_TELEPORTERS_LIST:
    {
        ...
        teleporters_no = SDL_SwapLE16 (*((uint16_t *)(in_data + 3)));
    ...
    case PONG:
    {
        ...
        safe_snprintf(str, sizeof(str), "%s: %i ms",server_latency, SDL_GetTicks()-SDL_SwapLE32(*((uint32_t *)(in_data+3))));
    ...
    case GET_NEW_BAG:
    {
        ...
        put_bag_on_ground(SDL_SwapLE16(*((uint16_t *)(in_data+3))), SDL_SwapLE16(*((uint16_t *)(in_data+5))), in_data[7]);
    }
        break;
    ...
    case FIRE_PARTICLES:
    {
        ...
        add_fire_at_tile (SDL_SwapLE16(*(uint16_t *)(in_data+7)), SDL_SwapLE16(*((uint16_t *)(in_data+3))), SDL_SwapLE16(*((uint16_t *)(in_data+5))));
    }
        break;
    ...
    case REMOVE_FIRE_AT:
    {
        ...
        remove_fire_at_tile (SDL_SwapLE16(*((uint16_t *)(in_data+3))),SDL_SwapLE16(*((uint16_t *)(in_data+5))));
    }
        break;
    ...
    case NPC_TEXT:
    {
        ...
        put_small_text_in_box((const char *)&in_data[3], data_length-3, dialogue_menu_x_len-70, (char*)dialogue_string);
        display_dialogue();
    ...
    case NPC_OPTIONS_LIST:
    {
        ...
        build_response_entries (in_data+3, SDL_SwapLE16 (*((uint16_t *)(in_data+1))));
    }
        break;
    ...
    {
        get_trade_partner_name(&in_data[3],SDL_SwapLE16(*((uint16_t *)(in_data+1)))-1);
    }
        break;
    ...
    case GET_YOUR_SIGILS:
    {
        ...
        if(data_length < 11){
            get_sigils_we_have(SDL_SwapLE32(*((uint32_t *)(in_data+3))), 0);
        } else {
            get_sigils_we_have(SDL_SwapLE32(*((uint32_t *)(in_data+3))), SDL_SwapLE32(*((uint32_t *)(in_data+7))));
        }
    }
        break;
    ...
    case GET_ACTOR_HEALTH:
    {
        ...
        get_actor_health(SDL_SwapLE16(*((uint16_t *)(in_data+3))),SDL_SwapLE16(*((uint16_t*)(in_data+5))));
    }
        break;
    ...
    case GET_ACTOR_DAMAGE:
    {
        ...
        while (data_length >= 7)
        {
            get_actor_damage(SDL_SwapLE16(*((uint16_t *)(in_data+3))),SDL_SwapLE16(*((uint16_t*)(in_data+5))));
            data_length -= 4;
            in_data += 4;
        }
#else
    ...
    case GET_ACTOR_HEAL:
    {
        ...
        while (data_length >= 7)
        {
            get_actor_heal(SDL_SwapLE16(*((uint16_t *)(in_data+3))),SDL_SwapLE16(*((uint16_t*)(in_data+5))));
            data_length -= 4;
            in_data += 4;
        }
#else
    ...
    case ACTOR_UNWEAR_ITEM:
    {
        ...
        unwear_item_from_actor(SDL_SwapLE16(*((uint16_t *)(in_data+3))),in_data[5]);
    }
        break;
    ...
    case ACTOR_WEAR_ITEM:
    {
        ...
        actor_wear_item(SDL_SwapLE16(*((uint16_t *)(in_data+3))),in_data[5],in_data[6]);
    }
        break;
    ...
    case NPC_SAY_OVERTEXT:
    {
        ...
        add_displayed_text_to_actor(
                    get_actor_ptr_from_id( SDL_SwapLE16(*((uint16_t *)(in_data+3))) ), buf);
    }
        break;
    ...
    case OPEN_BOOK:
    {
        ...
        open_book(SDL_SwapLE16(*((uint16_t*)(in_data+3))));
    }
        break;
    ...
    case CLOSE_BOOK:
    {
        ...
        close_book(SDL_SwapLE16(*((uint16_t*)(in_data+3))));
    }
        break;
    case STORAGE_LIST:
    {
    ...
    case GET_ACTIVE_CHANNELS:
        ...
        set_active_channels (in_data[3], (uint32_t*)(in_data+4), (data_length-2)/4);
        break;
    ...
    case REMOVE_3D_OBJ:
        ...
        remove_3d_object_from_server (SDL_SwapLE16 (*((uint16_t *)(&in_data[3]))));
        break;
    ...
    case MAP_FLAGS:
        ...
        map_flags=SDL_SwapLE32(*((uint32_t *)(in_data+3)));
        break;
...
#endif // BUFF_DEBUG
        update_actor_buffs(SDL_SwapLE16(*((short *)(in_data+3))), SDL_SwapLE32(*((uint32_t *)(in_data+5))));
        break;
    ...
    case SEND_SPECIAL_EFFECT:
        ...
        if (special_effects){
            parse_special_effect((special_effect_enum)in_data[3], (const uint16_t *) &in_data[4]);
        }
        break;
    ...
    case GET_NEW_MINE:
    {
        ...
        put_mine_on_ground(SDL_SwapLE16(*((uint16_t *)(in_data+3))), SDL_SwapLE16(*((uint16_t *)(in_data+5))), in_data[8], in_data[7]);
    }
        break;
    ...
    case SEND_MAP_MARKER:
    {
        ...
        sm = (server_mark *)calloc(1,sizeof(server_mark));//memory is set to zero
        ...
        hash_delete(server_marks,(void *)(sm->id)); //remove old marker if present
        hash_add(server_marks,(void *)(sm->id),(void*)sm);
    ...
    case REMOVE_MAP_MARKER:
    {
        ...
        hash_delete(server_marks,(void *)(id)); //remove marker if present
        save_server_markings();
    ...
    {
        uint32_t *achievement_data = NULL;
        size_t word_count = (data_length-3) / sizeof(uint32_t);
        size_t i;
        ...
        achievement_data = (uint32_t *)calloc(word_count, sizeof(uint32_t));
        for (i=0; i<word_count; ++i)
            achievement_data[i] = SDL_SwapLE32(*((uint32_t *)(in_data+3+i*sizeof(uint32_t))));
    ...
    case SEND_BUFF_DURATION:
    {
        if (data_length <= 3)
            ...
            here_is_a_buff_duration((uint8_t)in_data[3]);
        break;
    }
    default:
    {
        ...
        do { /* while (3 <= in_data_used) (enough data present for the length field) */
            ...
            if (sizeof (tcp_in_data) - 3 >= size) { /* buffer big enough ? */
                ...
                if (size <= in_data_used) { /* do we have a complete message ? */
                    ...
                    message->data = (uint8_t *)malloc(size*sizeof(uint8_t));
                    message->length = size;
...
int get_message_from_server(void *thread_args)
{
    ...
    int *done = (int *)(((void **) thread_args)[1]);
    ...
    while(!*done)
    {
        ...
        if ((received = SDLNet_TCP_Recv(my_socket, &tcp_in_data[in_data_used], sizeof (tcp_in_data) - in_data_used)) > 0) {
            ...
            process_data_from_server((queue_t *)queue);
        }
...
int my_tcp_send(TCPsocket my_socket, const uint8_t *str, int len) {
    return my_tcp_send(my_socket,(const char *)str,len);
}

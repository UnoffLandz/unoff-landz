...
class Achievements_Window
{
    public:
        ...
        void set_achievements(const std::vector<uint32_t> & data);
...
class Achievements_System
{
    public:
        ...
        void new_data(const uint32_t *data, size_t word_count);
...
//
void Achievements_System::new_data(const uint32_t *data, size_t word_count)
{
    ...
    if (achievement)
    {
        ...
        for (size_t i=0; i<achievement->get_text().size(); ++i)
            draw_string_small(as->get_border() + gx_adjust, (i + 1) * static_cast<int>(SMALL_FONT_Y_LEN) + gy_adjust,
                achievement->get_text()[i].c_str(), 1);
    }
    else
    {
        glColor3f(0.77f, 0.57f, 0.39f);
        ...
        draw_string_small(title_x + gx_adjust, as->get_border() + gy_adjust, buf.str().c_str(), 1);
    }
...
int Achievements_Window::display_handler(window_info *win)
{
    ...
    for (size_t i=first; i<their_achievements.size(); ++i)
    {
        ...
        if (missing)
        {
            ...
            draw_string(pos_x+gx_adjust, pos_y+gy_adjust, "?", 1);
        }
    }
...
//
int achievements_keypress_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
    if (!win)
        return 0;
...
//
static int achievements_click_handler(window_info *win, int mx, int my, uint32_t flags)
{
    if (!win || !win->data)
        return 0;
    if (my < 0)
...
//
void Achievements_Window::set_achievements(const std::vector<uint32_t> & data)
{
    ...
    for (size_t i=0; i<data.size(); ++i)
    {
        ...
        {
            if (word & 1)
                their_achievements.push_back(i*sizeof(uint32_t)*8+j);
            word >>= 1;
        }
    }
}
...
//
extern "C" void achievements_data(uint32_t *data, size_t word_count)
{

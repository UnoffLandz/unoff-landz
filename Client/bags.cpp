...
uint32_t ground_items_empty_next_bag=0;
...
void add_bags_from_list (const uint8_t *data)
{
    ...
    for(i=0;i<bags_no;i++) {
        my_offset=i*5+1;
        bag_x=SDL_SwapLE16(*((uint16_t *)(data+my_offset)));
        bag_y=SDL_SwapLE16(*((uint16_t *)(data+my_offset+2)));
        bag_id=*((uint8_t *)(data+my_offset+4));
        if(bag_id >= NUM_BAGS) {
            continue;
        }
        //now, get the Z position
...
void get_bag_item (const uint8_t *data)
{
    ...
    ground_item_list[pos].image_id= SDL_SwapLE16(*((uint16_t *)(data)));
    ground_item_list[pos].quantity= SDL_SwapLE32(*((uint32_t *)(data+2)));
...
void pick_up_all_items(void)
{
    ...
    for(itempos = 0; itempos < ITEMS_PER_BAG; itempos++){
        if(ground_item_list[itempos].quantity){
            str[0]=PICK_UP_ITEM;
            str[1]=itempos;
            *((uint32_t *)(str+2))=SDL_SwapLE32(ground_item_list[itempos].quantity);
...
void get_bags_items_list (const uint8_t *data)
{
    ...
    for(i=0;i<items_no;i++) {
        ...
        ground_item_list[pos].image_id= SDL_SwapLE16(*((uint16_t *)(data+my_offset)));
        ground_item_list[pos].quantity= SDL_SwapLE32(*((uint32_t *)(data+my_offset+2)));
        ground_item_list[pos].id= unset_item_uid;
        ground_item_list[pos].pos= pos;
    }
...
int display_ground_items_handler(window_info *win)
{
    ...
    draw_string_small(win->len_x-(GRIDSIZE-5), ELW_BOX_SIZE+3+yoffset, my_str, 2);
    ...
    for(i=ITEMS_PER_BAG-1; i>=0; --i) {
        if(ground_item_list[i].quantity > 0) {
            ...
            if ((mouseover_ground_item_pos == i) && enlarge_text())
                draw_string_shadowed(x_start,y_end-(i&1?22:12),str,1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
            else
                draw_string_small_shadowed(x_start,y_end-(i&1?22:12),str,1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
        }
    }
...
int click_ground_items_handler(window_info *win, int mx, int my, uint32_t flags)
{
    int pos;
    uint8_t str[10];
    ...
    if(!ground_item_list[pos].quantity) {
        ...
        str[0]= PICK_UP_ITEM;
        str[1]= ground_item_list[pos].pos;
        *((uint32_t *)(str+2))= SDL_SwapLE32(quantity);
        my_tcp_send(my_socket,str,6);
        do_get_item_sound();
    }
...
int mouseover_ground_items_handler(window_info *win, int mx, int my) {
    ...
    if(pos!=-1 && pos<ITEMS_PER_BAG && ground_item_list[pos].quantity) {
        uint16_t item_id = ground_item_list[pos].id;
...
void draw_pick_up_menu()
{
    if(ground_items_win < 0){
        ...
        set_window_handler(ground_items_win, ELW_HANDLER_CLICK, (int (*)())&click_ground_items_handler );
        set_window_handler(ground_items_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_ground_items_handler );

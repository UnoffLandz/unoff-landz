...
uint32_t loading_win = -1;
uint32_t loading_win_progress_bar = -1;
static float total_progress = 0;
GLuint loading_texture = 0;
float frac_x, frac_y;
#ifdef	NEW_TEXTURES
uint32_t loading_texture_handle;
uint32_t use_snapshot = 0;
#else	/* NEW_TEXTURES */
...
void update_loading_win (const char *text, float progress_increase)
{
    if(loading_win != -1) {
        ...
        {
            put_small_text_in_box(text, strlen(text), window_width, (char*)text_buffer);
        }

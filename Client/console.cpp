...
char *history_get_line_up(void)
{
    ...
    if(command_buffer_offset != NULL) {
        return (char *)command_buffer_offset->data;
    } else {
        return NULL;
    }
}
...
void add_command(const char *command, int (*callback)(const char *,int))
{
    ...
    if(command_count >= commands_size) {
        if(commands_size == 0) {
            commands_size += 10;
            commands = (command_t *)malloc(commands_size*sizeof(*commands));
        } else {
            commands_size *= 2;
            commands = (command_t *)realloc(commands, commands_size*sizeof(*commands));
        }
    }
...
void add_name_to_tablist(const char *name)
{
    ...
    if(name_count >= list_size) {
        ...
        list_size *= 2;
        name_list = (name_t *)realloc(name_list, list_size * sizeof(*name_list));
    }
...
struct compl_str tab_complete(const text_message *input, unsigned int cursor_pos)
{
    ...
    struct compl_str return_value = {NULL, UNKNOWN};
    ...
    if(input != NULL && input->len > 0 )
    {
        ...
        if(!have_last_complete || (*input_string && strncasecmp(input_string, last_complete, strlen(last_complete)) != 0)) {
            ...
            for(i = 0;
                ...
                !isspace((uint8_t)input_string[i]);
                i++) {
...
static const char *getparams(const char *text)
{
...
int print_emotes(const char *text, int len){
...
int command_cls(const char *text, int len)
{
    clear_display_text_buffer ();
    return 1;
}

int command_calc(const char *text, int len)
{
    double res;
    char str[100];
    int calcerr;
...
int command_markpos(const char *text, int len)
{
    int map_x, map_y;
    const char *ptr = text;
    char msg[512];
...
int command_unmark(const char *text, int len)
{
    return command_unmark_special(text, len, 1);
}


int command_mark_color(const char *text, int len)
{
    char str[512];
...
int command_stats(const char *text, int len)
{
...
int command_ping(const char *text, int len)
{
    ...
    str[0] = PING;
    *((uint32_t *)(str+1)) = SDL_SwapLE32(SDL_GetTicks());
...
int command_quit(const char *text, int len)
{
    exit_now = 1;
    return 1;
}

int command_mem(const char *text, int len)
{
...
int command_ver(const char *text, int len)
{
    char str[250];
...
int command_filter(const char *text, int len)
{
    char name[256];
    char str[100];
    int i;
...
int command_unignore(const char *text, int len)
{
    char name[MAX_USERNAME_LENGTH];
    char str[200];
    int i;
...
int command_unfilter(const char *text, int len)
{
    char name[64];
    char str[200];
    int i;
...
int command_glinfo (const char *text, int len)
{
    ...
    char* this_string = (char *)calloc (size, 1);
    ...
    if (size < minlen)
    {
        ...
        this_string = (char *)realloc (this_string, size);
    }
 ...
 */
int knowledge_command(const char *text, int len)
{
...
int command_log_conn_data(const char *text, int len)
{
    if(!log_conn_data){
...
int command_msg(const char *text, int len)
{
    int no;//, m=-1;
...
int command_afk(const char *text, int len)
{
    // find first space, then skip any spaces
    while(*text && !isspace(*text)) {
        text++;
...
int command_help(const char *text, int len)
{
...
int command_storage(const char *text, int len)
{
    int i;
    ...
    if (have_storage_list)
    {
        ...
        char cached_storage_copy[sizeof(cached_storage_list)];
        const char * endl;
        memcpy(cached_storage_copy, cached_storage_list, size);
        endl = (char*)strchr(cached_storage_copy, '\n');
        if (endl == NULL)
        {
...
int command_accept_buddy(const char *text, int len)
{
    ...
    if(*text && !queue_isempty(buddy_request_queue)) {
        ...
        while(node != NULL) {
            if(strcasecmp(text, (const char *)node->data) == 0) {
...
static int command_open_url(const char *text, int len)
{
    text = getparams(text);
    if (*text)
        open_web_link(text);
...
static int command_set_user_menu_wait_time_ms(const char *text, int len)
{
    text = getparams(text);
    if (*text)
...
static int command_cast_spell(const char *text, int len)
{
    int index = 0;
...
static int command_show_spell(const char *text, int len)
{
    int i;
    char out_str[128];
    char mess_str[64];
...
int command_ckdata(const char *text, int len)
{
...
int command_keypress(const char *text, int len)
{
    text = getparams(text);
    if (*text)
    {
        uint32_t value = get_key_value(text);
        if (value)
            do_keypress(value);
    }
    return 1;
...
static int session_counters(const char *text, int len)
{
    text = getparams(text);
...
static int command_relogin(const char *text, int len)
{

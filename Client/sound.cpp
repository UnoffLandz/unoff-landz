...
/* Stream handling */
static const char * get_stream_type(int type);
...
ALvoid * load_ogg_into_memory(char * szPath, ALenum *inFormat, ALsizei *inSize, ALfloat *inFreq)
{
    ...
    data = (char *)malloc(OGG_BUFFER_SIZE);
...
void add_source_to_lists(source_data *pSource)
{
    ...
    new_source_id = (source_list *)calloc(1, sizeof(source_list));
...
int play_sound(int sound_num, int x, int y, float initial_gain)
{
...
#endif // _EXTRA_SOUND_DEBUG
    ALuint buffer = 0;
    int stage; //SOUND_STAGE
    ALfloat sourcePos[] = {(ALfloat)x, (ALfloat)y, 0.0f};
    ...
    pSource->current_stage = (SOUND_STAGE)stage;
    ...
    for (; stage < (int)num_STAGES; ++stage)
    {
...
//			printf("'%s' has stopped after sample '%s'\n", pSoundType->name, pVariant->part[pSource->current_stage]->file_path);
        ...
        else
        {
            ...
            if (buffer != pSample->buffer)
            {
                ...
                k = 0;
#endif //_EXTRA_SOUND_DEBUG
                while ((pSource->current_stage = (SOUND_STAGE)((int)pSource->current_stage + 1)) <= num_STAGES)
                {
#ifdef _EXTRA_SOUND_DEBUG
                    k++;
                    if (k > num_STAGES)
...
void print_sound_boundaries(int map)
{
    ...
    draw_string_zoomed(25, 180, buf, 2, 0.4);

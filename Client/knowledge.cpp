...
int handle_knowledge_book(widget_list *widget, int mx, int my, uint32_t flags)
{
...
int display_knowledge_handler(window_info *win)
{
    ...
    draw_string_small(lx+points_pos,320,points_string,1);
    if (is_researching && mouse_over_progress_bar)
    {
        ...
        draw_string_small(lx+eta_pos,285,eta_string,1);
    ...
    for(i = 2*scroll; i < 2 * (scroll + 19); i++)
    {
        ...
        if ((get_string_width(knowledge_list[i].name) * font_ratio) > max_name_x)
        {
            ...
            draw_string_zoomed(x, y, used_name,1,font_ratio);
            ...
            draw_string_zoomed(x,y,knowledge_list[i].name,1,font_ratio);
...
int click_knowledge_handler(window_info *win, int mx, int my, uint32_t flags)
{
    int x,y,idx;
    uint8_t str[3];
        ...
        if(idx < knowledge_count)
            {
                ...
                *(uint16_t *)(str+1) = SDL_SwapLE16((short)idx);
...
void fill_knowledge_win ()
{
    ...
    set_window_handler(knowledge_win, ELW_HANDLER_CLICK, (int (*)())&click_knowledge_handler );
    set_window_handler(knowledge_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_knowledge_handler );

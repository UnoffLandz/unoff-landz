...
int click_map_handler (window_info *win, int mx, int my, uint32_t flags)
{
    uint32_t ctrl_on = flags & ELW_CTRL;
    uint32_t left_click = flags & ELW_LEFT_MOUSE;
    uint32_t right_click = flags & ELW_RIGHT_MOUSE;
...
int keypress_map_handler (window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
void create_map_root_window (int width, int height)
{
    if (map_root_win < 0)
    {
        ...
        set_window_handler (map_root_win, ELW_HANDLER_CLICK, (int (*)())&click_map_handler);
        set_window_handler (map_root_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_map_handler);

...
int display_astrology_handler(window_info *win)
{
    ...
    switch(astrology_display_type)
    {
        case adtTwoProgressBars:
        {
            ...
            draw_string_small((win->len_x >> 1 ) - strlen(stone_name)*4, 5, stone_name, 1);

            //draw the first indicator item
            draw_string_small (30, 30, text_item1, 1);
            //draw the second indicator item
            draw_string_small (30, 70, text_item2, 1);

            //draw the plus/minus
            draw_string_small (15, 47, "-", 1);
            draw_string_small (15, 87, "-", 1);

            draw_string_small (305, 47,"+", 1);
            draw_string_small (305, 87, "+", 1);
        }break;
        case adtThreeProgressBars:
        {
            ...
            draw_string_small (30, 110, text_item3, 1);

            //draw the plus/minus
            draw_string_small (15, 47, "-", 1);
            draw_string_small (15, 87, "-", 1);
            draw_string_small (15, 127, "-", 1);

            draw_string_small (305, 47, "+", 1);
            draw_string_small (305, 87, "+", 1);
            draw_string_small (305, 127, "+", 1);
        }break;
    }
...
int ok_handler(struct widget_list *,int , int, uint32_t)
{
...
void display_astrology_window(const char * raw_text)
{
    if(astrology_win < 0)
    {
        ...
        set_window_handler(astrology_win, ELW_HANDLER_DISPLAY, (int (*)())&display_astrology_handler );

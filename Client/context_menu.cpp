...
namespace cm
{
    ...
    class Menu
    {
        public:
            ...
            int display(window_info *win, int mx, int my, uint32_t flags);
    ...
    class Container
    {
        public:
            ...
            int pre_show_check(uint32_t flags);
    ...
    extern "C" int click_context_handler(window_info *win, int mx, int my, uint32_t flags)
    {
    ...
    extern "C" int display_context_handler(window_info *win, int mx, int my, uint32_t flags)
    {
        Menu *thismenu = (Menu *)win->data;
    ...
    int Container::pre_show_check(uint32_t flags)
    {
    ...
    int Menu::display(window_info *win, int mx, int my, uint32_t flags)
    {
        CHECK_GL_ERRORS();
        ...
        for (size_t i=0; i<menu_lines.size(); ++i)
        {
            ...
            else
            {
                ...
                // draw the text
                draw_string_zoomed(int(border+text_border+bool_tick_width+0.5), int(currenty+0.5), menu_lines[i].text.c_str(), 1, zoom);
                currenty += line_step;
            }
    ...
    int Menu::click(window_info *win, int mx, int my, uint32_t flags)
    {
        if (selection < 0)
            return 0;
...
extern "C" int cm_pre_show_check(uint32_t flags) { return cm::container.pre_show_check(flags); }
...
static int cm_info_but_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    cm_showinfo();
    return 1;
}

static int cm_dir_click_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
...
static int cm_add_win_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    printf("Adding window cm_add_window()=%d\n", cm_add_window(cm_test_win_menu, widget->window_id));
    return 1;
}

extern "C" int cm_del_win_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    printf("Removing window cm_remove_window()=%d\n", cm_remove_window(widget->window_id));
    return 1;
}

static int cm_add_reg_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    int i;
    for (i=0; i<2; i++)
...
extern "C" int cm_del_reg_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    printf("Removing regions status=%d\n", cm_remove_regions(widget->window_id));
    return 1;
}

static int cm_add_wid_handler(widget_list *widget, int mx, int my, uint32_t flags)
{
    printf("Adding widget cm_add_widget()=%d\n", cm_add_widget(cm_test_wid_menu, widget->window_id, cm_test_wid));
    return 1;
}

extern "C" int cm_del_wid_handler(widget_list *widget, int mx, int my, uint32_t flags)
{

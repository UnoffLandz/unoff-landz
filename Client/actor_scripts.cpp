...
static uint32_t update_actor_animation_speed(actor* a, const float time_diff)
{
    float scale, seconds;
    uint32_t i, animations;
...
/* 					break; */
                ...
                case aim_mode_fire:
                    ...
                    missiles_fire_arrow(actors_list[i], action->fire_position, (MissileShotType)action->shot_type);
...
int push_command_in_actor_queue(actor_commands command, actor *act)
{
    int k;
    for(k=0;k<MAX_CMD_QUEUE;k++){
...
//if ((actor_id==yourself)&&(command==enter_combat)) LOG_TO_CONSOLE(c_green2,"FIGHT!");
    ...
    if(!act){
        ...
        k = push_command_in_actor_queue((actor_commands)command, act);
        ...
        if (act->attached_actor >= 0){
            ...
            switch(command){
                ...
                case aim_mode_fire:
                    ...
                    break;
                default:
                k2 = push_command_in_actor_queue((actor_commands)command, actors_list[act->attached_actor]);
            }
        }
        else
            k2 = k;
...
void free_emote_data(void *data){
    emote_data *emote=(emote_data *)data;
    ...
    for (item = node->children; item; item = item->next) {
        if (item->type == XML_ELEMENT_NODE) {
            if (xmlStrcasecmp (item->name, (xmlChar*)"anim") == 0) {
                ...
                i=j=k=0;
                if(!head){
                    head=(emote_frame *)calloc(1,sizeof(emote_frame));
                    frames=head;
                } else {
                    frames->next=(emote_frame *)calloc(1,sizeof(emote_frame));
                    frames=frames->next;
                }
        ...
        if (item->type == XML_ELEMENT_NODE) {
            ...
            if (xmlStrcasecmp (item->name, (xmlChar*)"CAL_IDLE_GROUP") == 0) {
                ...
                if(!strncasecmp(etag,(const char*) item->name,9)) {
                    ...
                    anim=(cal_anim *)calloc(1,sizeof(cal_anim));
    ...
    if(defaults){
        ...
        if(default_node){
            ...
            if(boots->model_name==NULL || *boots->model_name=='\0'){
                get_item_string_value(boots->model_name, sizeof(boots->model_name), default_node, "mesh");
                boots->mesh_index= cal_load_mesh(act, boots->model_name, "boots");
            }
        }
    }

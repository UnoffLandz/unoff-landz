...
void missiles_rotate_actor_bones(actor *a)
{
    CalSkeleton *skel;
    CalBone *bone;
    CalQuaternion *bone_rot, *bone_rot_abs, *hrot_quat, *vrot_quat;
    CalVector *vect;
...
void missiles_test_target_validity(float target[3], const char *msg)
{
...
void missiles_fire_xyz_to_b(float *origin, int actor_id)
{
    ...
    missiles_add(0, origin, target, 0.0, MissileShotType::MISSED_SHOT);
}
...
int display_range_handler(window_info *win)
{
    ...
    draw_string_small(pos_x, pos_y, str,2);
    pos_y += step_y;

...
int	click_misc_handler(window_info *win, int mx, int my, uint32_t flags);
int	mouseover_misc_handler(window_info *win, int mx, int my);
int	display_quickbar_handler(window_info *win);
int	click_quickbar_handler(window_info *win, int mx, int my, uint32_t flags);
...
void show_sized_help_coloured(const char *help_message, int x, int y, float r, float g, float b, int big)
{
    ...
    glColor3f(r,g,b);
    if (big)
        draw_string(x, y, help_message, 1);
    else
...
// calculate the number of digits for the specified uint32_t
static uint32_t Uint32_digits(uint32_t number)
{
    uint32_t digits = 1;
    long step = 10;
...
static int recalc_exp_bar_text_len(void)
{
    ...
    static uint32_t last_exp[NUM_WATCH_STAT-1];
    static uint32_t last_to_go_len[NUM_WATCH_STAT-1];
    static uint32_t last_selected[NUM_WATCH_STAT-1];
    int recalc = 1;
    int i;
...
void init_stats_display()
{
    ...
    if(stats_bar_win < 0)
    {
        ...
        set_window_handler(stats_bar_win, ELW_HANDLER_DISPLAY, (int (*)())&display_stats_bar_handler);
        set_window_handler(stats_bar_win, ELW_HANDLER_MOUSEOVER,(int (*)()) &mouseover_stats_bar_handler);
...
void draw_stats_bar(int x, int y, int val, int len, float r, float g, float b, float r2, float g2, float b2)
{
    ...
    draw_string_small_shadowed(x-(1+8*strlen(buf))-1, y-2, buf, 1,0.8f, 0.8f, 0.8f,0.0f,0.0f,0.0f);
#ifdef OPENGL_TRACE
CHECK_GL_ERRORS();
#endif //OPENGL_TRACE
}
...
static struct { int d; int h; uint32_t dt; uint32_t ht; } my_last_health = { 0, 0, 0, 0 };
...
{
    char str[20];
    static const uint32_t timeoutms = 2*60*1000;
...
void init_misc_display(hud_interface type)
{
    ...
    if(misc_win < 0)
        ...
        set_window_handler(misc_win, ELW_HANDLER_CLICK, (int (*)())&click_misc_handler);
        set_window_handler(misc_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_misc_handler );
...
CHECK_GL_ERRORS();
    ...
    if(view_digital_clock > 0){
        ...
        if (show_game_seconds)
        {
            ...
            draw_string_shadowed_width(5, 4 + base_y_start,str, win->len_x-5, 1, 0.77f, 0.57f, 0.39f, 0.0f, 0.0f, 0.0f);
        }
        else
        {
            safe_snprintf(str, sizeof(str), "%1d:%02d", real_game_minute/60, real_game_minute%60);
            x= 3+(win->len_x - (get_string_width(str)*11)/12)/2;
            draw_string_shadowed(x, 2 + base_y_start, str, 1,0.77f, 0.57f, 0.39f,0.0f,0.0f,0.0f);
        }
    }
    ...
    if (mouse_over_clock)
    {
        ...
        safe_snprintf(str, sizeof(str), "%1d:%02d:%02d", real_game_minute/60, real_game_minute%60, real_game_second);
        draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(str)+0.5)), centre_y-SMALL_FONT_Y_LEN, str, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f, 0.0f);
        if (the_date != NULL)
        {
            safe_snprintf(str, sizeof(str), "%s", the_date);
            draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(str)+0.5)), centre_y, str, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f, 0.0f);
        }
        mouse_over_clock = 0;
    }
    ...
    if (mouse_over_compass)
    {
        ...
        if (me != NULL)
        {
            ...
            draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(str)+0.5)), win->len_y-64, str, 1, 1.0f, 1.0f, 1.0f, 0.0f, 0.0f, 0.0f);
        }
    ...
    if (view_knowledge_bar)
    {
        ...
        draw_string_small_shadowed(x+off+gx_adjust, y+gy_adjust,use_str, 1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);

        if (mouse_over_knowledge_bar)
        {
            use_str = (is_researching()) ?get_research_eta_str(str, sizeof(str)) : not_researching_str;
            draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(use_str)+0.5)), y+gy_adjust, use_str, 1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
...
CHECK_GL_ERRORS();
        ...
        for (thestat=0; thestat<NUM_WATCH_STAT-1; thestat++)
        {
            ...
            if (statsinfo[thestat].is_selected == 1)
                draw_string_small_shadowed(x+gx_adjust, y+gy_adjust, str, 1,0.77f, 0.57f, 0.39f,0.0f,0.0f,0.0f);
            else
                draw_string_small_shadowed(x+gx_adjust, y+gy_adjust, str, 1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
            ...
            if((thestat!=NUM_WATCH_STAT-2) && floatingmessages_enabled &&
                ...
                if(skill_modifier > 0){
                    draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(str)+0.5)), y+gy_adjust, str, 1,0.3f, 1.0f, 0.3f,0.0f,0.0f,0.0f);
                } else {
                    draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(str)+0.5)), y+gy_adjust, str, 1,1.0f, 0.1f, 0.2f,0.0f,0.0f,0.0f);
                }
            }
            ...
            if (stat_mouse_is_over == thestat)
            {
                ...
                draw_string_small_shadowed(-(int)(SMALL_FONT_X_LEN*(strlen(str)+0.5+hover_offset)), y+gy_adjust, str, 1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
...
{
    size_t i;
    uint32_t keys[] = {K_ITEM1, K_ITEM2, K_ITEM3, K_ITEM4, K_ITEM5, K_ITEM6,
...
void init_quickbar ()
{
    uint32_t flags = ELW_USE_BACKGROUND | ELW_USE_BORDER;
    ...
    if (quickbar_win < 0)
    {
        ...
        set_window_handler(quickbar_win, ELW_HANDLER_CLICK, (int (*)())&click_quickbar_handler);
        set_window_handler(quickbar_win, ELW_HANDLER_MOUSEOVER, (int (*)())&mouseover_quickbar_handler );
...
int	display_quickbar_handler(window_info *win)
{
    char str[80];
    int y, i;
    uint32_t _cur_time = SDL_GetTicks(); /* grab a snapshot of current time */
    int ypos = -1, xpos = -1;
    ...
    for(i=num_quickbar_slots-1;i>=0;i--)
    {
        if(item_list[i].quantity > 0)
        {
            ...
            if ((mouseover_quickbar_item_pos == i) && enlarge_text())
                draw_string_shadowed(xpos,ypos,str,1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
            else
                draw_string_small_shadowed(xpos,ypos,str,1,1.0f,1.0f,1.0f,0.0f,0.0f,0.0f);
        }
    }
...
{
    uint16_t item_id = item_list[pos].id;
                            ...
                            if(item_list[i].quantity && item_list[i].pos==y)
                                {
                                    ...
                                    if(ctrl_on){
                                        ...
                                        *((uint32_t *)(str+2))=item_list[i].quantity;
...
void toggle_quickbar_draggable()
{
    uint32_t flags = get_flags(quickbar_win);
    if (!quickbar_draggable)
    {
        flags &= ~ELW_SHOW_LAST;
...
void draw_exp_display()
{
    ...
    for (i=0; i<max_disp_stats; i++)
    {
        if (watch_this_stats[i] > 0)
        {
            ...
            const char * name = statsinfo[watch_this_stats[i]-1].skillnames->name;
            ...
            name_x = my_exp_bar_start_x + stats_bar_len - strlen(name) * SMALL_FONT_X_LEN;
            ...
            if (name_x < icon_x)
            {
                ...
                else
                {
                    ...
                    name_x = my_exp_bar_start_x + stats_bar_len - strlen(name) * SMALL_FONT_X_LEN - 3;
                    name_y = -3;
                }
            }
...
void handle_stats_selection(int stat, uint32_t flags)
{
    int i;

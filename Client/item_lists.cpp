...
namespace ItemLists
{
    ...
    static int click_itemlist_handler(window_info *win, int mx, int my, uint32_t flags);
    static int mouseover_itemlist_handler(window_info *win, int mx, int my);
    static int hide_itemlist_handler(window_info *win);
    static int keypress_itemlist_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey);
    ...
    class List
    {
        public:
            ...
            uint16_t get_item_id(size_t i) const { assert(i<item_ids.size()); return item_ids[i]; }
            ...
            void add(size_t over_item_number, int image_id, uint16_t id, int quantity);
    ...
    class List_Container
    {
        public:
            ...
            void add_item(size_t over_item_number, int image_id, uint16_t id, int quantity)
    ...
    class Category_Maps
    {
        public:
            ...
            void update(int image_id, uint16_t item_id, int cat_id);
            bool have_image_id(int image_id) const
                { return cat_by_image_id.find(image_id) != cat_by_image_id.end(); }
            bool have_item_id(uint16_t item_id) const
                { return cat_by_item_id.find(item_id) != cat_by_item_id.end(); }
            int get_cat(int image_id, uint16_t item_id);
            void save(void);
            void load(void);
        private:
            static const char *filename;
            std::map<int, int> cat_by_image_id;
            std::map<uint16_t, int> cat_by_item_id;
            bool must_save;
            bool loaded;
            struct IDS { public: std::vector<int> images; std::vector<uint16_t> items; };
    };
    ...
    class List_Window
    {
        public:
            ...
            int click(window_info *win, int mx, int my, uint32_t flags);
    ...
    void List::add(size_t over_item_number, int image_id, uint16_t id, int quantity)
    {
        do_drop_item_sound();
    ...
    //
    void Category_Maps::update(int image_id, uint16_t item_id, int cat_id)
    {
        if (!loaded)
            load();
    ...
    //
    int Category_Maps::get_cat(int image_id, uint16_t item_id)
    {
        if ((item_id != unset_item_uid) && have_item_id(item_id))
        {
    ...
    void Category_Maps::save(void)
    {
        ...
        for (std::map<uint16_t,int>::const_iterator i=cat_by_item_id.begin(); i!=cat_by_item_id.end(); ++i)
        ...
        for (std::map<int,IDS>::const_iterator i=ids_in_cat.begin(); i!=ids_in_cat.end(); ++i)
        {
            ...
            for (std::vector<uint16_t>::const_iterator j=i->second.items.begin(); j!=i->second.items.end(); ++j)
                out << *j << " ";
    ...
    int List_Window::draw(window_info *win)
    {
        ...
        if (Vars::lists()->valid_active_list())
        {
            ...
            for(size_t i=0; i<Vars::lists()->get_list().get_num_items() && i<static_cast<size_t>(6*num_grid_rows); i++)
            {
                ...
                draw_string_small_shadowed(x_start, (i&1)?(y_end-15):(y_end-27), str, 1,1.0f,1.0f,1.0f, 0.0f, 0.0f, 0.0f);
            }
        }
        ...
        if (mouse_over_add_button)
            ...
            glColor3f(0.77f,0.57f,0.39f);
        draw_string_zoomed(add_button_x, add_button_y, "+", 1, 2.0);
        ...
        for (size_t i = top_entry; i<lists.size() && num_shown<num_show_names_list; ++i)
        {
            ...
            if (lists[i].get_name().size() > disp_chars)
            {
                ...
                draw_string_small(get_list_gap(), pos_y, todisp.c_str(), 1);
                ...
                draw_string_small(get_list_gap(), pos_y, lists[i].get_name().c_str(), 1);
...
CHECK_GL_ERRORS();
    ...
    int List_Window::mouseover(window_info *win, int mx, int my)
    {
            ...
            mx>=0 && mx<(get_grid_size()*6) && my>=0 && my<(get_grid_size()*num_grid_rows))
        {
            ...
            {
                uint16_t item_id = Vars::lists()->get_list().get_item_id(item_number);
    ...
    //
    int List_Window::click(window_info *win, int mx, int my, uint32_t flags)
    {
        ...
        if ((flags & ELW_RIGHT_MOUSE) || (flags & ELW_LEFT_MOUSE))
        {
            if ((over_item_number!=last_selected) && (over_item_number < num_items))
            {
                ...
                if (flags & ELW_LEFT_MOUSE)
                {
                    ...
                    uint16_t item_id = Vars::lists()->get_list().get_item_id(selected_item_number);
...
extern "C"
{
    ...
    void update_category_maps(int image_id, uint16_t item_id, int cat_id)

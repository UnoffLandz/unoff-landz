...
{
    const _buddy *b1=(const _buddy *)arg1, *b2=(const _buddy *)arg2;
    if(b1->type==b2->type)
...
int click_buddy_handler (window_info *win, int mx, int my, uint32_t flags)
{
    int x=mx,y=my;
    char str[50];
    ...
    if(x>win->len_x-20) {
        ...
        while(!queue_isempty(buddy_request_queue)) {
            char *name = (char *)queue_pop(buddy_request_queue);
...
int click_add_buddy_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    if(strlen((char*)buddy_name_buffer) == 0) {
        ...
        my_tcp_send(my_socket, (uint8_t*)string, strlen(string+1)+1);
...
int click_change_buddy_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    char string[255];
    int send_message = 1;
...
int name_input_keypress_handler(widget_list *widget, int mx, int my, uint32_t key, uint32_t unikey)
{
...
int click_accept_yes(widget_list *w, int mx, int my, uint32_t flags)
{
    char string[255];
...
int click_accept_no(widget_list *w, int mx, int my, uint32_t flags)
{
    int i;
    int window_id = w->window_id;
...
int click_delete_checkbox_label(widget_list *w, int mx, int my, uint32_t flags)
{
...
int create_buddy_interface_win(const char *title, void *argument)
{
    ...
    if(strcmp(title, buddy_add_str) == 0) {
        ...
        accept_windows[current_window].text = (char *)malloc((strlen(string)+5)*sizeof(*accept_windows[current_window].text));
...
int click_buddy_button_handler(widget_list *w, int mx, int my, uint32_t flags)
{
    if(buddy_add_win < 0) {
    ...
    if(buddy_win < 0)
        {
            ...
            set_window_handler(buddy_win, ELW_HANDLER_CLICK,(int (*)()) &click_buddy_handler );
...
int is_in_buddylist(const char *name)
{
    ...
    while(name[0] != '\0' && is_color ((uint8_t)name[0])){
        ++name;
    }

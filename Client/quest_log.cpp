...
{
    public:
        Quest(uint16_t id) : is_completed(false), title("") { this->id = id; }
        ...
        uint16_t get_id(void) const { return id; }
        void write(std::ostream & out) const
            { out << id << " " << is_completed << " " << title << std::endl; }
        static const uint16_t UNSET_ID;
    private:
        uint16_t id;
        bool is_completed;
        std::string title;
...
{
    public:
        Quest_Title_Request(uint16_t req_id) : id(req_id), requested(false) {}
        uint16_t get_id(void) const { return id; }
        void request(void);
...
class Quest_List
{
    public:
        ...
        void load(void);
        void save(void);
        void set_completed(uint16_t id, bool is_complete);
        bool get_completed(uint16_t id) const;
        void toggle_completed(uint16_t id) { set_completed(id, !get_completed(id)); }
        size_t num_shown(void) const { return number_shown; }
        const Quest * get_first_quest(int offset);
        const Quest * get_next_quest(void);
        void set_selected(uint16_t id) { selected_id = id; }
        uint16_t get_selected(void) const { return selected_id; }
        ...
        void set_highlighted(uint16_t id) { highlighted_id = id; }
        uint16_t get_highlighted(void) const { return highlighted_id; }
    ...
    private:
        std::map <uint16_t,Quest,QuestCompare> quests;
        std::queue <Quest_Title_Request> title_requests;
        bool save_needed;
        std::string list_filename;
        std::map <uint16_t,Quest,QuestCompare>::const_iterator iter;
        bool iter_set;
        size_t max_title;
        uint16_t selected_id;
        uint16_t highlighted_id;
        int win_id;
        int scroll_id;
        int mouseover_y;
        bool clicked;
...
const uint16_t Quest::UNSET_ID = static_cast<uint16_t>(-1);
...
//
void Quest_List::add(uint16_t id)
{
...
//
void Quest_List::set_completed(uint16_t id, bool is_complete)
{
    if (id == Quest::UNSET_ID)
        return;
    std::map<uint16_t,Quest,QuestCompare>::iterator i = quests.find(id);
    if (i != quests.end())
...
bool Quest_List::get_completed(uint16_t id) const
{
    if (id == Quest::UNSET_ID)
        return false;
    std::map<uint16_t,Quest,QuestCompare>::const_iterator i = quests.find(id);
    if (i != quests.end())
...
void Quest_List::set_requested_title(const char* title)
{
    ...
    std::map<uint16_t,Quest,QuestCompare>::iterator i = quests.find(title_requests.front().get_id());
    if (i != quests.end())
    {
...
void Quest_List::load(void)
{
    ...
    while (getline(in, line))
    {
        ...
        if (new_quest.get_id() != Quest::UNSET_ID)
        {
            ...
            std::map<uint16_t,Quest,QuestCompare>::iterator i = quests.find(new_quest.get_id());
            if ((i != quests.end()))
            {
...
void Quest_List::save(void)
{
    ...
    for (std::map<uint16_t,Quest,QuestCompare>::const_iterator i=quests.begin(); i!=quests.end(); ++i)
...
class Quest_Entry
{
    public:
        ...
        uint16_t get_charsum(void) const { return charsum; }
        void set_id(uint16_t id) { quest_id = id; }
        uint16_t get_id(void) const { return quest_id; }
...
static uint16_t next_entry_quest_id = Quest::UNSET_ID;
...
void Quest_Entry::set_lines(const std::string & the_text)
{
    ...
    if (close == text.size()-2)
    {
        ...
        if (open != std::string::npos)
        {
            ...
            {
                quest_id = static_cast<uint16_t>(atoi(id_str.c_str()));
...
static int display_quest_filter_handler(window_info *win)
{
    ...
    for (std::map<std::string,int>::const_iterator i = filter_map.begin(); i != filter_map.end(); ++i)
    {
        ...
        // draw the string
        draw_string_small(posx + npc_name_box_size + npc_name_space, posy, i->first.c_str(), 1);
...
//
static int click_quest_filter_handler(window_info *win, int mx, int my, uint32_t flags)
{
...
//
static int keypress_quest_filter_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
...
static int display_questlist_handler(window_info *win)
{
    ...
    while ((thequest != 0) && (posy+questlist.get_linesep()-questlist.get_spacer() <= win->len_y))
    {
        ...
        if (thequest->get_title().size() > disp_chars)
        {
            ...
            draw_string_small(2*questlist.get_spacer(), posy, todisp.c_str(), 1);
        }
        else if (thequest->get_title().empty())
            draw_string_small(2*questlist.get_spacer(), posy, "???", 1);
        else
            draw_string_small(2*questlist.get_spacer(), posy, thequest->get_title().c_str(), 1);
...
//
static int click_questlist_handler(window_info *win, int mx, int my, uint32_t flags)
{
...
{
    uint16_t setected_id = questlist.get_selected();
...
void delete_duplicates(void)
{
    ...
    std::multimap<uint16_t,std::string> mm;
    ...
    for (std::vector<Quest_Entry>::iterator entry=quest_entries.begin(); entry!=quest_entries.end(); ++entry)
    {
        if (!entry->get_deleted())
        {
            ...
            std::multimap<uint16_t,std::string>::const_iterator iter = mm.find(entry->get_charsum());
            if (iter != mm.end())
            {
                // we have a charsum match so check the text, there may be more than one with this charsum
                std::multimap<uint16_t,std::string>::const_iterator last = mm.upper_bound(entry->get_charsum());
                for ( ; iter != last; ++iter)
                {
            ...
            if (!entry->get_deleted())
                mm.insert(std::pair<uint16_t,std::string>(entry->get_charsum(), the_text));
        }
    }
...
static int cm_quest_handler(window_info *win, int widget_id, int mx, int my, int option)
{
    ...
    switch (option)
    {
        ...
        case CMQL_JUSTTHISQUEST:
            ...
            {
                uint16_t the_id = quest_entries[active_entries[over_entry]].get_id();
...
static int display_questlog_handler(window_info *win)
{
    ...
    for (std::vector<Quest_Entry>::size_type entry = current_line; entry<active_entries.size(); entry++)
    {
        ...
        {
            draw_string_small (qlborder+gx_adjust, questlog_y+gy_adjust, line->c_str(), 1);
        ...
        if ((cm_questlog_over_entry < active_entries.size()) && (entry == cm_questlog_over_entry))
        {
            ...
            draw_string_small(qlborder+gx_adjust, start_y+gy_adjust,
                quest_entries[active_entries[entry]].get_disp_npc().c_str(), 1);
        }
...
static int questlog_scroll_click (widget_list *widget, int mx, int my, uint32_t flags)
{
...
static int questlog_scroll_drag (widget_list *widget, int mx, int my, uint32_t flags, int dx, int dy)
{
    int scroll = vscrollbar_get_pos (questlog_win, widget->id);
    goto_questlog_entry (scroll);
    return 1;
...
static int questlog_click(window_info *win, int mx, int my, uint32_t flags)
{
    if(flags&ELW_WHEEL_UP) {
...
static int keypress_questlog_handler(window_info *win, int mx, int my, uint32_t key, uint32_t unikey)
{
    char keychar = tolower(static_cast<char>(unikey));
...
extern "C" void set_next_quest_entry_id(uint16_t id)
{
    //char buf[80];
...
extern "C" void set_quest_finished(uint16_t id)
{
    //char buf[80];

...
int	drag_in_window(int win_id, int x, int y, uint32_t flags, int dx, int dy);
int	mouseover_window(int win_id, int x, int y);	// do mouseover processing for a window
int	keypress_in_window(int win_id, int x, int y, uint32_t key, uint32_t unikey);	// keypress in the window
...
int	click_in_windows(int mx, int my, uint32_t flags)
{
    int	done= 0;
    int	id;
    int	next_id;
...
int	drag_in_windows(int mx, int my, uint32_t flags, int dx, int dy)
{
    int	done = 0;
    int	id;
    int	next_id;
...
int	keypress_in_windows(int x, int y, uint32_t key, uint32_t unikey)
{
    int	done= 0;
    int	id;
    int	next_id;
...
int	create_window(const char *name, int pos_id, uint32_t pos_loc, int pos_x, int pos_y, int size_x, int size_y, uint32_t property_flags)
{
    window_info *win;
    int	win_id=-1;
    int	i;
...
int	init_window(int win_id, int pos_id, uint32_t pos_loc, int pos_x, int pos_y, int size_x, int size_y)
{
    int pwin_x, pwin_y;
    ...
    if(windows_list.window[win_id].flags&ELW_SCROLLABLE) {
        ...
        uint16_t x = size_x-ELW_BOX_SIZE,
                y = size_y,
                width = ELW_BOX_SIZE,
                height = size_y;
...
int	move_window(int win_id, int pos_id, uint32_t pos_loc, int pos_x, int pos_y)
{
    window_info *win;
    int dx, dy, i;
...
CHECK_GL_ERRORS();
    ...
    while(W != NULL)
    {
        if (!(W->Flags&WIDGET_INVISIBLE) && !(W->Flags&WIDGET_DISABLED)) {
            ...
            {
                if(W->spec != NULL)
                    ((int (*)(widget_list *,void *))W->OnDraw)(W, W->spec);
                else
                    W->OnDraw(W);
            }
        }
...
void show_window(int win_id)
{
    ...
    if (win->after_show_handler)
        (*win->after_show_handler)(win);
}
...
int	click_in_window(int win_id, int x, int y, uint32_t flags)
{
    window_info *win;
    int	mx, my;
    widget_list *W;
...
int	keypress_in_window(int win_id, int x, int y, uint32_t key, uint32_t unikey)
{
    window_info *win;
    int	mx, my;
    int scroll_pos = 0;
...
void	*set_window_handler(int win_id, int handler_id, int (*handler)(void *) ) {
    return set_window_handler(win_id,handler_id,(int (*)())handler);
}
...
int	set_window_color(int win_id, uint32_t color_id, float r, float g, float b, float a)
{
    if(win_id < 0 || win_id >= windows_list.num_windows)	return 0;
    if(windows_list.window[win_id].window_id != win_id)	return 0;
...
int	use_window_color(int win_id, uint32_t color_id)
{
    if(win_id < 0 || win_id >= windows_list.num_windows)	return 0;
    if(windows_list.window[win_id].window_id != win_id)	return 0;
...
int set_window_flag (int win_id, uint32_t flag)
{
    if (win_id < 0 || win_id >= windows_list.num_windows)	return 0;
    if (windows_list.window[win_id].window_id != win_id)	return 0;

...
const uint16_t unset_item_uid = (uint16_t)-1;
...
void get_your_items (const uint8_t *data)
{
    ...
    for(i=0;i<total_items;i++){
        ...
        if(item_list[pos].image_id != SDL_SwapLE16(*((uint16_t *)(data+i*len+1))) ){
            item_list[pos].cooldown_time = 0;
            item_list[pos].cooldown_rate = 1;
        }
        item_list[pos].image_id=SDL_SwapLE16(*((uint16_t *)(data+i*len+1)));
        item_list[pos].quantity=SDL_SwapLE32(*((uint32_t *)(data+i*len+1+2)));
        ...
        if (item_uid_enabled)
            item_list[pos].id=SDL_SwapLE16(*((uint16_t *)(data+i*len+1+8)));
        else
...
void get_new_inventory_item (const uint8_t *data)
{
    ...
    if (item_uid_enabled)
        id=SDL_SwapLE16(*((uint16_t *)(data+8)));
    else
        id=unset_item_uid;

    pos= data[6];
    flags= data[7];
    image_id=SDL_SwapLE16(*((uint16_t *)(data)));
    quantity=SDL_SwapLE32(*((uint32_t *)(data+2)));
...
int display_items_handler(window_info *win)
{
    ...
    */
    ...
    for (i=0; i<NUMBUT; i++) {
        ...
        draw_string_small(win->len_x+gx_adjust-XLENBUT, wear_items_y_offset+but_y_off[i]+2+gy_adjust, my_str, 2);
    }
    ...
    for(i=0;i<ITEM_EDIT_QUANT;x+=quantity_width,++i){
        if(i==edit_quantity){
            ...
            draw_string_small(1+gx_adjust+x-strlen(quantities.quantity[i].str)*4, y+gy_adjust, quantities.quantity[i].str, 1);
            glColor3f(0.3f, 0.5f, 1.0f);
        } else if(i==quantities.selected){
            glColor3f(0.0f, 1.0f, 0.3f);
            draw_string_small(1+gx_adjust+x-strlen(quantities.quantity[i].str)*4, y+gy_adjust, quantities.quantity[i].str, 1);
            glColor3f(0.3f, 0.5f, 1.0f);
        } else draw_string_small(1+gx_adjust+x-strlen(quantities.quantity[i].str)*4, y+gy_adjust, quantities.quantity[i].str, 1);
    }
    draw_string_small(win->len_x-strlen(quantity_str)*8-5, quantity_y_offset-19, quantity_str, 1);

    glColor3f(0.57f,0.67f,0.49f);
    draw_string_small (wear_items_x_offset + 33 - (8 * strlen(equip_str))/2, wear_items_y_offset-18, equip_str, 1);
        ...
        if(item_list[i].quantity){
            ...
            if(!item_is_weared){
                ...
                if ((mouseover_item_pos == i) && enlarge_text())
                    draw_string_shadowed(x_start, (i&1)?(y_end-15):(y_end-25), str, 1,1.0f,1.0f,1.0f, 0.0f, 0.0f, 0.0f);
                else
                    draw_string_small_shadowed(x_start, (i&1)?(y_end-15):(y_end-25), str, 1,1.0f,1.0f,1.0f, 0.0f, 0.0f, 0.0f);
            }
        }
    }
    ...
    if (!use_small_items_window)
    {
        ...
        draw_string_small(items_grid_size*6+6, items_grid_size*6-SMALL_FONT_Y_LEN*2, str, 1);
        safe_snprintf(str, sizeof(str), "%i/%i", your_info.carry_capacity.cur, your_info.carry_capacity.base);
        draw_string_small(items_grid_size*6+6, items_grid_size*6-SMALL_FONT_Y_LEN, str, 1);
    }
    else
    {
        ...
        draw_string_small(2, quantity_y_offset-19, str, 1);
    }

    //now, draw the inventory text, if any.
    if (last_items_string_id != inventory_item_string_id)
    {
        put_small_text_in_box(inventory_item_string, strlen(inventory_item_string), win->len_x-10, items_string);
        last_items_string_id = inventory_item_string_id;
    }
    draw_string_small(4, win->len_y - (use_small_items_window?105:85), items_string, 4);
...
int click_items_handler(window_info *win, int mx, int my, uint32_t flags)
{
    uint8_t str[100];
    ...
    //see if we clicked on any item in the main category
        ...
        if(pos==-1) {
            ...
            str[0]=WITHDRAW_ITEM;
            *((uint16_t*)(str+1))=SDL_SwapLE16(storage_items[storage_item_dragged].pos);
            *((uint32_t*)(str+3))=SDL_SwapLE32(item_quantity);
            ...
            if (ctrl_on && (items_mod_click_any_cursor || (item_action_mode==ACTION_WALK))) {
                ...
                if(item_list[pos].is_stackable)
                    *((uint32_t *)(str+2))=SDL_SwapLE32(item_list[pos].quantity);
                else
                    *((uint32_t *)(str+2))=SDL_SwapLE32(36);//Drop all
                my_tcp_send(my_socket, str, 6);
                do_drop_item_sound();
                ...
                if ((storage_win >= 0) && (get_show_window(storage_win)) && (view_only_storage == 0)) {
                    str[0]=DEPOSITE_ITEM;
                    str[1]=item_list[pos].pos;
                    *((uint32_t*)(str+2))=SDL_SwapLE32(INT_MAX);
                    my_tcp_send(my_socket, str, 6);
                    do_drop_item_sound();
                } else {
...
int keypress_items_handler(window_info * win, int x, int y, uint32_t key, uint32_t keysym)
{
    if(edit_quantity!=-1){
    ...
    if (safe_button_click(&last_click))
    {
        for(i = 0; i < ITEM_NUM_ITEMS; i++)
        {
                ...
                item_list[i].pos < ((items_dropall_nolastrow)?30:ITEM_WEAR_START))
            {
                ...
                *((uint32_t *)(str+2)) = SDL_SwapLE32(item_list[i].quantity);
                my_tcp_send (my_socket, str, 6);
                dropped_something = 1;
            }
        }
...
void display_items_menu()
{
    if(items_win < 0){
        ...
        set_window_handler(items_win, ELW_HANDLER_CLICK, (int (*)(void *))&click_items_handler );
        set_window_handler(items_win, ELW_HANDLER_MOUSEOVER, (int (*)(void *))&mouseover_items_handler );
        set_window_handler(items_win, ELW_HANDLER_KEYPRESS, (int (*)(void *))&keypress_items_handler );
...
void get_items_cooldown (const uint8_t *data, int len)
{
    ...
    {
        pos = data[ibyte];
        max_cooldown = SDL_SwapLE16 (*((uint16_t*)(&data[ibyte+1])));
        cooldown = SDL_SwapLE16 (*((uint16_t*)(&data[ibyte+3])));
        ibyte += 5;

        item_list[pos].cooldown_rate = 1000 * (uint32_t)max_cooldown;
        item_list[pos].cooldown_time = cur_time + 1000 * (uint32_t)cooldown;
    }
}

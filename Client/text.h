...
#define CHAT_ALL	((uint8_t) -1)
#define CHAT_NONE	((uint8_t) -2)
#define CHAT_LIST	((uint8_t) -3)
#define CHAT_HIST	((uint8_t) -4)
...
extern uint32_t last_server_message_time; /*!< timestamp of the last server message */
 ...
 */
static __inline__ void init_text_message (text_message *msg, uint16_t size)
{
 ...
 */
void send_input_text_line (const char *line, int len);
 ...
 *
 *      put_small_text_in_box(char*,int,int,char*)
 *
 * \param text_to_add   the string to add
 * \param len           the length of text_to_add
 * \param pixels_limit
 ...
 *
 *      put_small_colored_text_in_box(char*,int,int,char*)
 *
 ...
 *
 *      find_last_lines_time(int *, int *, uint8_t filter, int width)
 *
 * \retval int
 *
 * \todo Fix documentation
...
#define LOG_TO_CONSOLE(color,buffer)	put_colored_text_in_buffer(color,CHAT_SERVER,(const char*)buffer,-1) /*!< logs the text in buffer with the specified color to the console. */

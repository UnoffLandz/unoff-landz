...
void remove_chat_tab (uint8_t channel);
int add_chat_tab (int nlines, uint8_t channel);
void update_chat_tab_idx (uint8_t old_ix, uint8_t new_idx);
void remove_tab_button (uint8_t channel);
int add_tab_button (uint8_t channel);
void update_tab_button_idx (uint8_t old_idx, uint8_t new_idx);
void convert_tabs (int new_wc);
int display_channel_color_win(uint32_t channel_number);

uint32_t active_channels[MAX_ACTIVE_CHANNELS];
...
void update_tab_idx (uint8_t old_idx, uint8_t new_idx)
{
    // XXX: CAUTION
...
void set_channel_tabs (const uint32_t *chans)
{
    int nmax = CHAT_CHANNEL3-CHAT_CHANNEL1+1;
    uint32_t chan;
    uint8_t chan_nr, chan_nrp;
...
{
    uint32_t tmp[MAX_ACTIVE_CHANNELS];
    int i;
...
void send_active_channel (uint8_t chan)
{
    uint8_t msg[2];
...
chan_name *tab_label (uint8_t chan);//Forward declaration
...
void clear_input_line (void)
{
    ...
    if(input_widget != NULL) {
        text_field *field = (text_field *)input_widget->widget_info;
        field->cursor = 0;
        field->cursor_line = 0;
        field->nr_lines = 1;
...
void update_chat_tab_idx (uint8_t old_idx, uint8_t new_idx)
{
    int itab;
...
uint8_t get_tab_channel (uint8_t channel)
{
    switch (channel)
    {
        case CHAT_LOCAL:
...
void switch_to_chat_tab(int id, char click)
{
    if(!click)
    {
        ...
        tab_collection *collection = (tab_collection *)widget->widget_info;
...
int chat_tabs_click (widget_list *widget, int mx, int my, uint32_t flags)
{
    int id;
...
int chat_scroll_drag (widget_list *widget, int mx, int my, uint32_t flags, int dx, int dy)
{
...
int chat_scroll_click (widget_list *widget, int mx, int my, uint32_t flags)
{
    int line = vscrollbar_get_pos (chat_win, widget->id);
    if (line != current_line)
    {
...
int root_key_to_input_field (uint32_t key, uint32_t unikey)
{
    ...
    tf = (text_field *)input_widget->widget_info;
...
void create_chat_window(void)
{
    ...
    set_window_handler (chat_win, ELW_HANDLER_RESIZE, (int (*)())&resize_chat_handler);
...
void add_chan_name(int no, const char * name, const char * desc)
{
    chan_name *entry;
    int len;

    if(((entry = (chan_name *)malloc(sizeof(*entry))) == NULL)
        ||((entry->description = (char *)malloc(strlen(desc)+1)) == NULL)
        ||((entry->name = (char *)malloc(strlen(name)+1)) == NULL)) {
...
void add_spec_chan_name(int no, const char * name, const char * desc)
{
    chan_name *entry;
    if(((entry = (chan_name *)malloc(sizeof(*entry))) == NULL)
        ||((entry->description = (char *)malloc(strlen(desc)+1)) == NULL)
...
int tab_bar_button_click (widget_list *w, int mx, int my, uint32_t flags)
{
    int itab;
...
chan_name *tab_label (uint8_t chan)
{
    ...
    return (chan_name *)step->next->data;
}

unsigned int chan_int_from_name(const char * name, int * return_length)
{
    node_t *step = queue_front_node(chan_name_queue);
...
int chan_tab_mouseover_handler(widget_list *widget,int mx, int my)
{
    int itab = 0;
...
int click_chan_sel_handler(window_info *win, int mx, int my, uint32_t flags)
{
    int i = 0, y = my-5;
    node_t *step = queue_front_node(chan_name_queue);
    y /= 18;
...
int tab_special_click(widget_list *w, int mx, int my, uint32_t flags)
{
    int itab = 0;
    for (itab = 0; itab < tabs_in_use; itab++) {
            ...
            switch(tabs[itab].channel) {
                ...
                case CHAT_LIST:
                    if(chan_sel_win >= 0) {
                        ...
                        set_window_handler (chan_sel_win, ELW_HANDLER_CLICK, (int (*)())&click_chan_sel_handler);
...
int command_jlc(const char *text, int len)
{
    ...
    // FIXME: modifying const char * contents !!!
        safe_strncpy((char *)text, number, strlen(text));	//could be longer than the name, and hence we may
...
int click_channel_color_handler(widget_list *w, int mx, int my, uint32_t flags)
{
